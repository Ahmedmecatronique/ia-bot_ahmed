    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.IA_BOT_AHMED_CHANNEL === "prod" &&
...
const deepLinkEvent = "ia-bot-ahmed:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__IA_BOT_AHMED__ ??= {}
  const pending = window.__IA_BOT_AHMED__.deepLinks ?? []
  window.__IA_BOT_AHMED__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("ia-bot-ahmed.global.dat").getItem("language")
