...
const main = Effect.gen(function* () {
  ...
  const onboardingTestRoot = ((): string | undefined => {
    ...
    const root = join(tmpdir(), `ia-bot-ahmed-onboarding-${randomUUID()}`)
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("ia-bot-ahmed://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("ia-bot-ahmed")
  registerRendererProtocol()
  setDockIcon()
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.IA_BOT_AHMED_PORT
    if (fromEnv) {
