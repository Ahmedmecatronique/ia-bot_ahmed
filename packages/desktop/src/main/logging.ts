...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `ia-bot-ahmed-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "ia-bot-ahmed", "log"), join(app.getPath("userData"), "ia-bot-ahmed", "log")])]
}
