...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve ia-bot-ahmed route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "ia-bot-ahmed")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "ia-bot-ahmed")).toBe("openai")
    expect(statProvider("big-pickle", "", "ia-bot-ahmed")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "ia-bot-ahmed"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
