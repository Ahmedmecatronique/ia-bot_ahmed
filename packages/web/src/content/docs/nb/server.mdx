---
title: Tjener
description: Samhandle med ia-bot-ahmed-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `ia-bot-ahmed serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en ia-bot-ahmed-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `ia-bot-ahmed.local` |
...
Sett `IA_BOT_AHMED_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `ia-bot-ahmed` som standard, eller sett `IA_BOT_AHMED_SERVER_USERNAME` for å overstyre det. Dette gjelder både `ia-bot-ahmed serve` og `ia-bot-ahmed web`.
...
Når du kjører `ia-bot-ahmed` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk ia-bot-ahmed-serveren til å samhandle med ia-bot-ahmed programmatisk.
:::

Denne arkitekturen lar ia-bot-ahmed støtte flere klienter og lar deg samhandle med ia-bot-ahmed programmatisk.

Du kan kjøre `ia-bot-ahmed serve` for å starte en frittstående server. Hvis du har
ia-bot-ahmed TUI kjører, vil `ia-bot-ahmed serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av ia-bot-ahmed [IDE](/docs/ide) plugins.
...
ia-bot-ahmed-serveren viser følgende APIer.
