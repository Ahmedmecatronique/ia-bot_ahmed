---
title: Konfigurasjon
description: Ved å bruke ia-bot-ahmed JSON-konfigurasjonen.
---

Du kan konfigurere ia-bot-ahmed ved å bruke en JSON konfigurasjonsfil.

---

## Format

ia-bot-ahmed støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "ia-bot-ahmed"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/ia-bot-ahmed`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`IA_BOT_AHMED_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`ia-bot-ahmed.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.ia-bot-ahmed` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`IA_BOT_AHMED_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.ia-bot-ahmed` og `~/.config/ia-bot-ahmed` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/ia-bot-ahmed`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale ia-bot-ahmed-konfigurasjon i `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `ia-bot-ahmed.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når ia-bot-ahmed starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `IA_BOT_AHMED_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `IA_BOT_AHMED_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.ia-bot-ahmed` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.ia-bot-ahmed`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `ia-bot-ahmed serve` og `ia-bot-ahmed web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage ia-bot-ahmed-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `ia-bot-ahmed.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i ia-bot-ahmed-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver ia-bot-ahmed å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/ia-bot-ahmed/agents/` eller `.ia-bot-ahmed/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil ia-bot-ahmed falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`ia-bot-ahmed run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/ia-bot-ahmed/commands/` eller `.ia-bot-ahmed/commands/`. [Les mer her](/docs/commands).
...
ia-bot-ahmed vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater ia-bot-ahmed alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider ia-bot-ahmed med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.ia-bot-ahmed/plugins/` eller `~/.config/ia-bot-ahmed/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense ia-bot-ahmed til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
