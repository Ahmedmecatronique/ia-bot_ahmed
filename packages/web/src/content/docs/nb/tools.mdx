...
Verktøy lar LLM utføre handlinger i kodebasen din. ia-bot-ahmed kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i ia-bot-ahmed.
...
:::note
Dette verktøyet er bare tilgjengelig når `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (eller `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker ia-bot-ahmed-leverandøren eller når miljøvariabelen `IA_BOT_AHMED_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter ia-bot-ahmed:
