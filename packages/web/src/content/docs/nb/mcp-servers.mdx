...
Du kan legge til eksterne verktøy i ia-bot-ahmed ved å bruke _Model Context Protocol_ eller MCP. ia-bot-ahmed støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="ia-bot-ahmed.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/ia-bot-ahmed`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="ia-bot-ahmed.jsonc" {15}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
ia-bot-ahmed håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil ia-bot-ahmed:
...
Hvis serveren krever autentisering, vil ia-bot-ahmed be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `ia-bot-ahmed mcp auth <server-name>`.
...
```json title="ia-bot-ahmed.json" {7-11}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
ia-bot-ahmed mcp auth my-oauth-server
```
...
```bash
ia-bot-ahmed mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil ia-bot-ahmed lagre tokens sikkert i `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
```json title="ia-bot-ahmed.json" {7}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "my-api-key-server": {
...
ia-bot-ahmed mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i ia-bot-ahmed, sammen med innebygde verktøy. Så du kan administrere dem gjennom ia-bot-ahmed-konfigurasjonen som alle andre verktøy.
...
```json title="ia-bot-ahmed.json" {14}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="ia-bot-ahmed.json" {11, 14-18}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="ia-bot-ahmed.json" {4-8}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "sentry": {
...
```bash
ia-bot-ahmed mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble ia-bot-ahmed til Sentry-kontoen din.
...
```json title="ia-bot-ahmed.json" {4-7}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "context7": {
...
```json title="ia-bot-ahmed.json" {7-9}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver ia-bot-ahmed-samtale oppført som en separat økt.
