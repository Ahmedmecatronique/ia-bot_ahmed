---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i ia-bot-ahmed-konfigurasjonen.
...
i `~/.local/share/ia-bot-ahmed/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i ia-bot-ahmed
konfig.
...
ia-bot-ahmed Zen er en liste over modeller levert av ia-bot-ahmed-teamet som har vært
testet og verifisert for å fungere godt med ia-bot-ahmed. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med ia-bot-ahmed Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `ia-bot-ahmed Zen` og gå til [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Det fungerer som alle andre leverandører i ia-bot-ahmed og er helt valgfritt å bruke.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av ia-bot-ahmed-teamet som har vært
testet og verifisert for å fungere godt med ia-bot-ahmed.

1. Kjør kommandoen `/connect` i TUI, velg `ia-bot-ahmed Go`, og gå til [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Slik bruker du Amazon Bedrock med ia-bot-ahmed:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører ia-bot-ahmed:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `ia-bot-ahmed.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i ia-bot-ahmed støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere ia-bot-ahmed til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at ia-bot-ahmed skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom ia-bot-ahmed-konfigurasjonen.
...
for å lagre token i ia-bot-ahmed auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
ia-bot-ahmed bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse ia-bot-ahmed
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`ia-bot-ahmed.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `ia-bot-ahmed.json`:
...
Slik bruker du GitHub Copilot-abonnementet med ia-bot-ahmed:
...
Slik bruker du Google Vertex AI med ia-bot-ahmed:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører ia-bot-ahmed.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom ia-bot-ahmed, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) plugin for automatisk å logge hver ia-bot-ahmed-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver ia-bot-ahmed-samtale oppført som en separat økt.
...
Du kan konfigurere ia-bot-ahmed for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere ia-bot-ahmed for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere ia-bot-ahmed for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for ia-bot-ahmed. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/ia-bot-ahmed) for detaljer.
:::
...
Slik bruker du Ollama Cloud med ia-bot-ahmed:
...
3. Kopier API-nøkkelen for bruk i ia-bot-ahmed.
...
6. **Viktig**: Før du bruker skymodeller i ia-bot-ahmed, må du hente modellinformasjonen lokalt:
...
ia-bot-ahmed Zen er en liste over testede og verifiserte modeller levert av ia-bot-ahmed-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>ia-bot-ahmed Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Skriv inn ia-bot-ahmed API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom ia-bot-ahmed-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom ia-bot-ahmed-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med ia-bot-ahmed:
...
Du kan også tilpasse modeller gjennom ia-bot-ahmed-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med ia-bot-ahmed. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `ia-bot-ahmed.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar ia-bot-ahmed forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `ia-bot-ahmed auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk ia-bot-ahmed-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i ia-bot-ahmed-konfigurasjonen.
