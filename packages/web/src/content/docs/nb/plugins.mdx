...
description: Skriv dine egne programtillegg for å utvide ia-bot-ahmed.
---

Plugins lar deg utvide ia-bot-ahmed ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Global konfigurasjon (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Prosjektkonfigurasjon (`ia-bot-ahmed.json`)
3. Global plugin-katalog (`~/.config/ia-bot-ahmed/plugins/`)
4. Prosjektpluginkatalog (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En ia-bot-ahmed SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide ia-bot-ahmed.
...
:::note
Hvis du bruker ia-bot-ahmed-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre ia-bot-ahmed fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til ia-bot-ahmed:
...
`tool`-hjelperen lager et tilpasset verktøy som ia-bot-ahmed kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for ia-bot-ahmed sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://ia-bot-ahmed.app/docs/sdk) for detaljer.
