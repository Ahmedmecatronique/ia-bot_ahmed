...
工具允许 LLM 在您的代码库中执行操作。ia-bot-ahmed 自带一组内置工具，您也可以通过[自定义工具](/docs/custom-tools)或 [MCP 服务器](/docs/mcp-servers)来扩展它。
...
:::note
该工具仅在设置 `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true`（或 `IA_BOT_AHMED_EXPERIMENTAL=true`）时可用。
:::
...
:::note
该工具仅在使用 ia-bot-ahmed 提供商时，或当 `IA_BOT_AHMED_ENABLE_EXA` 环境变量设置为任意真值（例如 `true` 或 `1`）时可用。

在启动 ia-bot-ahmed 时启用：
