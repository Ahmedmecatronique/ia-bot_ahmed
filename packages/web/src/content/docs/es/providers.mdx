---
title: Proveedores
description: Usando cualquier proveedor LLM en ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración ia-bot-ahmed.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu ia-bot-ahmed
configuración.
...
ia-bot-ahmed Zen es una lista de modelos proporcionados por el equipo ia-bot-ahmed que han sido
probado y verificado para funcionar bien con ia-bot-ahmed. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con ia-bot-ahmed Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione ia-bot-ahmed y diríjase a [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/auth).
...
Funciona como cualquier otro proveedor en ia-bot-ahmed y su uso es completamente opcional.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de ia-bot-ahmed que han sido
probado y verificado para funcionar bien con ia-bot-ahmed.

1. Ejecute el comando `/connect` en TUI, seleccione `ia-bot-ahmed Go` y diríjase a [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Para usar Amazon Bedrock con ia-bot-ahmed:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta ia-bot-ahmed:
   ...
   Para una configuración persistente o específica del proyecto, utilice `ia-bot-ahmed.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en ia-bot-ahmed no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar ia-bot-ahmed para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que ia-bot-ahmed funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración ia-bot-ahmed.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `ia-bot-ahmed`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación ia-bot-ahmed.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
ia-bot-ahmed utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear ia-bot-ahmed
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `ia-bot-ahmed.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `ia-bot-ahmed.json`:
...
Para utilizar su suscripción GitHub Copilot con ia-bot-ahmed:
...
Para utilizar Google Vertex AI con ia-bot-ahmed:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta ia-bot-ahmed.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de ia-bot-ahmed, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) para registrar automáticamente cada conversación ia-bot-ahmed como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación ia-bot-ahmed enumerada como una sesión separada.
...
Puede configurar ia-bot-ahmed para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar ia-bot-ahmed para usar modelos locales a través de LM Studio.
...
Puedes configurar ia-bot-ahmed para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para ia-bot-ahmed. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/ia-bot-ahmed) para obtener más detalles.
:::
...
Para usar Ollama Cloud con ia-bot-ahmed:
...
3. Copie la clave API para usarla en ia-bot-ahmed.
...
6. **Importante**: Antes de usar modelos en la nube en ia-bot-ahmed, debe obtener la información del modelo localmente:
...
ia-bot-ahmed Zen es una lista de modelos probados y verificados proporcionada por el equipo ia-bot-ahmed. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>ia-bot-ahmed Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave ia-bot-ahmed API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración ia-bot-ahmed.
...
5. También puedes personalizarlos a través de tu configuración ia-bot-ahmed. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con ia-bot-ahmed:
...
También puede personalizar modelos a través de su configuración ia-bot-ahmed. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con ia-bot-ahmed. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `ia-bot-ahmed.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a ia-bot-ahmed comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `ia-bot-ahmed auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración ia-bot-ahmed y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración ia-bot-ahmed.
