...
Puede agregar herramientas externas a ia-bot-ahmed utilizando el _Model Context Protocol_ o MCP. ia-bot-ahmed admite servidores locales y remotos.
...
Puede definir servidores MCP en su [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/ia-bot-ahmed`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
ia-bot-ahmed maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, ia-bot-ahmed:
...
Si el servidor requiere autenticación, ia-bot-ahmed le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `ia-bot-ahmed mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, ia-bot-ahmed almacenará los tokens de forma segura en `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en ia-bot-ahmed, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración ia-bot-ahmed como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar ia-bot-ahmed a su cuenta Sentry.
