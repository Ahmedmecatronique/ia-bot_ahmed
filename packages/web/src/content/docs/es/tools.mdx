...
Las herramientas permiten que LLM realice acciones en su código base. ia-bot-ahmed viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en ia-bot-ahmed.
...
```json title="ia-bot-ahmed.json" {4}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "permission": {
    "bash": "allow"
...
:::note
Esta herramienta solo está disponible cuando `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (o `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor ia-bot-ahmed o cuando la variable de entorno `IA_BOT_AHMED_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar ia-bot-ahmed:

```bash
IA_BOT_AHMED_ENABLE_EXA=1 ia-bot-ahmed
```
