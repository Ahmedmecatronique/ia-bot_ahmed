...
Para depurar problemas con ia-bot-ahmed, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\ia-bot-ahmed\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `ia-bot-ahmed --log-level DEBUG`.

---

## Almacenamiento

ia-bot-ahmed almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/ia-bot-ahmed/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\ia-bot-ahmed`
...
ia-bot-ahmed Desktop ejecuta un servidor ia-bot-ahmed local (el sidecar `ia-bot-ahmed-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `ia-bot-ahmed` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/ia-bot-ahmed/ia-bot-ahmed.jsonc` (o `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/ia-bot-ahmed/ia-bot-ahmed.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\ia-bot-ahmed\ia-bot-ahmed.jsonc`
...
ia-bot-ahmed también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/ia-bot-ahmed/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\ia-bot-ahmed\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que ia-bot-ahmed pueda reconstruirlo.

1. Salga de ia-bot-ahmed Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/ia-bot-ahmed`
- **Linux**: eliminar `~/.cache/ia-bot-ahmed` (o ejecutar `rm -rf ~/.cache/ia-bot-ahmed`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\ia-bot-ahmed`

3. Reinicie el escritorio ia-bot-ahmed.

---

### Solucionar problemas de conexión del servidor

ia-bot-ahmed Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `ia-bot-ahmed.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `IA_BOT_AHMED_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `IA_BOT_AHMED_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio ia-bot-ahmed requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de ia-bot-ahmed.

---

### Notificaciones que no se muestran

ia-bot-ahmed Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para ia-bot-ahmed en la configuración de su sistema operativo, y
...
1. Salga del escritorio ia-bot-ahmed.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio ia-bot-ahmed):

- `ia-bot-ahmed.settings.dat` (URL del servidor predeterminado de escritorio)
- `ia-bot-ahmed.global.dat` y `ia-bot-ahmed.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con ia-bot-ahmed:
...
3. Asegúrese de tener la última versión con `ia-bot-ahmed upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `ia-bot-ahmed models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\ia-bot-ahmed`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. ia-bot-ahmed instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\ia-bot-ahmed`

2. Reinicie ia-bot-ahmed para reinstalar los paquetes del proveedor más recientes.

Esto obligará a ia-bot-ahmed a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
ia-bot-ahmed detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
