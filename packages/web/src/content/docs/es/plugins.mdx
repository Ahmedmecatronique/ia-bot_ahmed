---
title: Complementos
description: Escriba sus propios complementos para extender ia-bot-ahmed.
---

Los complementos le permiten extender ia-bot-ahmed al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - Complementos a nivel de proyecto
- `~/.config/ia-bot-ahmed/plugins/` - Complementos globales
...
```json title="ia-bot-ahmed.json"
{
  ...
  "plugin": ["ia-bot-ahmed-helicone-session", "ia-bot-ahmed-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Configuración global (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Configuración del proyecto (`ia-bot-ahmed.json`)
3. Directorio global de complementos (`~/.config/ia-bot-ahmed/plugins/`)
4. Directorio de complementos del proyecto (`.ia-bot-ahmed/plugins/`)
...
```json title=".ia-bot-ahmed/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
ia-bot-ahmed ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".ia-bot-ahmed/plugins/my-plugin.ts"
...
```js title=".ia-bot-ahmed/plugins/example.js"
...
- `client`: Un cliente SDK ia-bot-ahmed para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar ia-bot-ahmed.
...
```js title=".ia-bot-ahmed/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "ia-bot-ahmed"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio ia-bot-ahmed, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que ia-bot-ahmed lea archivos `.env`:

```javascript title=".ia-bot-ahmed/plugins/env-protection.js"
...
```javascript title=".ia-bot-ahmed/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a ia-bot-ahmed:

```ts title=".ia-bot-ahmed/plugins/custom-tools.ts"
import { type Plugin, tool } from "@ia-bot-ahmed/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que ia-bot-ahmed puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para ia-bot-ahmed junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://ia-bot-ahmed.app/docs/sdk) para obtener más detalles.
...
```ts title=".ia-bot-ahmed/plugins/compaction.ts"
import type { Plugin } from "@ia-bot-ahmed/plugin"
...
```ts title=".ia-bot-ahmed/plugins/custom-compaction.ts"
import type { Plugin } from "@ia-bot-ahmed/plugin"
