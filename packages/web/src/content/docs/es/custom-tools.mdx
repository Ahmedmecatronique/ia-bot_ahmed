...
description: Cree herramientas que LLM pueda llamar en ia-bot-ahmed.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de ia-bot-ahmed como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.ia-bot-ahmed/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/ia-bot-ahmed/tools/`.
...
```ts title=".ia-bot-ahmed/tools/database.ts" {1}
...
```ts title=".ia-bot-ahmed/tools/math.ts"
import { tool } from "@ia-bot-ahmed/plugin"
...
```ts title=".ia-bot-ahmed/tools/bash.ts"
import { tool } from "@ia-bot-ahmed/plugin"
...
```ts title=".ia-bot-ahmed/tools/project.ts" {8}
import { tool } from "@ia-bot-ahmed/plugin"
...
```python title=".ia-bot-ahmed/tools/add.py"
import sys
...
```ts title=".ia-bot-ahmed/tools/python-add.ts" {10}
import { tool } from "@ia-bot-ahmed/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".ia-bot-ahmed/tools/add.py")
