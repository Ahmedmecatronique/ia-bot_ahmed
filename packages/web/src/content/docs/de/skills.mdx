...
Agent Skills erlauben ia-bot-ahmed, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
ia-bot-ahmed sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft ia-bot-ahmed vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.ia-bot-ahmed/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/ia-bot-ahmed/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.ia-bot-ahmed/skills/git-release/SKILL.md` zum Beispiel so:
...
ia-bot-ahmed listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `ia-bot-ahmed.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `ia-bot-ahmed.json`):
