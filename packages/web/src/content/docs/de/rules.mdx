---
title: Regeln
description: Lege eigene Anweisungen fuer ia-bot-ahmed fest.
---

Du kannst ia-bot-ahmed ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in ia-bot-ahmed den Befehl `/init` aus.
...
So kann ia-bot-ahmed sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht ia-bot-ahmed sie zu erweitern.
...
ia-bot-ahmed kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/ia-bot-ahmed/AGENTS.md` speichern.
Diese gelten in allen ia-bot-ahmed-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt ia-bot-ahmed dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/ia-bot-ahmed/AGENTS.md` existiert)
...
Beim Start sucht ia-bot-ahmed in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/ia-bot-ahmed/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `ia-bot-ahmed.json` oder in `~/.config/ia-bot-ahmed/ia-bot-ahmed.json` angeben.
...
ia-bot-ahmed parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit ia-bot-ahmed.json

Empfohlen ist das `instructions`-Feld in der `ia-bot-ahmed.json`:
...
Du kannst ia-bot-ahmed explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `ia-bot-ahmed.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
