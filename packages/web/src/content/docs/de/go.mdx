...
ia-bot-ahmed Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in ia-bot-ahmed. Du abonnierst ia-bot-ahmed Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
ia-bot-ahmed zu verwenden.
...
ia-bot-ahmed Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

ia-bot-ahmed Go funktioniert wie jeder andere Provider in ia-bot-ahmed.

1. Du meldest dich bei **<a href={console}>ia-bot-ahmed Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `ia-bot-ahmed Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann ia-bot-ahmed Go abonnieren.
:::
...
ia-bot-ahmed Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner ia-bot-ahmed Config verwendet das Format `ia-bot-ahmed-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `ia-bot-ahmed-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben ia-bot-ahmed Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit ia-bot-ahmed zu nutzen.
