---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um ia-bot-ahmed zu erweitern.
---

Mit Plugins können Sie ia-bot-ahmed erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von ia-bot-ahmed zu ändern.
...
- `.ia-bot-ahmed/plugins/` – Plugins auf Projektebene
- `~/.config/ia-bot-ahmed/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/ia-bot-ahmed/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Projektkonfiguration (`ia-bot-ahmed.json`)
3. Globales Plugin-Verzeichnis (`~/.config/ia-bot-ahmed/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein ia-bot-ahmed-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie ia-bot-ahmed erweitern können.
...
:::note
Wenn Sie die ia-bot-ahmed-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass ia-bot-ahmed `.env`-Dateien liest:
...
Plugins können ia-bot-ahmed auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das ia-bot-ahmed aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für ia-bot-ahmed zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://ia-bot-ahmed.app/docs/sdk).
