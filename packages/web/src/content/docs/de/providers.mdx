---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer ia-bot-ahmed-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem ia-bot-ahmed anpassen
config.
...
ia-bot-ahmed Zen ist eine Liste von Modellen, die vom ia-bot-ahmed-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit ia-bot-ahmed funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit ia-bot-ahmed Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie ia-bot-ahmed aus und gehen Sie zu [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/auth).
...
Es funktioniert wie jeder andere Anbieter in ia-bot-ahmed und ist völlig optional.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom ia-bot-ahmed-Team getestet und verifiziert wurden, dass sie gut mit ia-bot-ahmed funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `ia-bot-ahmed Go` und gehen Sie zu [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
So verwenden Sie Amazon Bedrock mit ia-bot-ahmed:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie ia-bot-ahmed ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `ia-bot-ahmed.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in ia-bot-ahmed wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können ia-bot-ahmed so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit ia-bot-ahmed ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre ia-bot-ahmed-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `ia-bot-ahmed`, Bereiche: `api`
...
um das Token im ia-bot-ahmed-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
ia-bot-ahmed verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von ia-bot-ahmed
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`ia-bot-ahmed.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Anpassen über `ia-bot-ahmed.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit ia-bot-ahmed:
...
So verwenden Sie Google Vertex AI mit ia-bot-ahmed:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie ia-bot-ahmed ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über ia-bot-ahmed konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session)-Plugin, um jede ia-bot-ahmed-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede ia-bot-ahmed-Konversation als separate Sitzung aufgeführt.
...
Sie können ia-bot-ahmed für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können ia-bot-ahmed über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können ia-bot-ahmed über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für ia-bot-ahmed konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/ia-bot-ahmed).
:::
...
So verwenden Sie Ollama Cloud mit ia-bot-ahmed:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in ia-bot-ahmed.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in ia-bot-ahmed verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
ia-bot-ahmed Zen ist eine Liste getesteter und verifizierter Modelle, die vom ia-bot-ahmed-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>ia-bot-ahmed Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren ia-bot-ahmed API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre ia-bot-ahmed-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre ia-bot-ahmed-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit ia-bot-ahmed:
...
Sie können Modelle auch über Ihre ia-bot-ahmed-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit ia-bot-ahmed verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `ia-bot-ahmed.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann ia-bot-ahmed erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `ia-bot-ahmed auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die ia-bot-ahmed-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer ia-bot-ahmed-Konfiguration übereinstimmt.
