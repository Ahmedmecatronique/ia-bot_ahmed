...
description: Erstellen Sie Tools, die der LLM in ia-bot-ahmed aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von ia-bot-ahmed wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.ia-bot-ahmed/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/ia-bot-ahmed/tools/` platzieren.
