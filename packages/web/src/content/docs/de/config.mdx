---
title: Konfiguration
description: Verwenden Sie die Konfiguration ia-bot-ahmed JSON.
---

Sie können ia-bot-ahmed mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

ia-bot-ahmed unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Wenn Ihre globale Konfiguration beispielsweise `theme: "ia-bot-ahmed"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/ia-bot-ahmed`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`IA_BOT_AHMED_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`ia-bot-ahmed.json` im Projekt) – projektspezifische Einstellungen
5. **`.ia-bot-ahmed` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`IA_BOT_AHMED_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.ia-bot-ahmed` und `~/.config/ia-bot-ahmed` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/ia-bot-ahmed` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale ia-bot-ahmed-Konfiguration in `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `ia-bot-ahmed.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn ia-bot-ahmed gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `IA_BOT_AHMED_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `IA_BOT_AHMED_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.ia-bot-ahmed` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.ia-bot-ahmed` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `ia-bot-ahmed serve` und `ia-bot-ahmed web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres ia-bot-ahmed-Servers erkannt werden.
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `ia-bot-ahmed.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer ia-bot-ahmed-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht ia-bot-ahmed, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer ia-bot-ahmed-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/ia-bot-ahmed/agents/` oder `.ia-bot-ahmed/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift ia-bot-ahmed mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`ia-bot-ahmed run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/ia-bot-ahmed/commands/` oder `.ia-bot-ahmed/commands/` definieren. [Mehr erfahren](/docs/commands).
...
ia-bot-ahmed lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
ia-bot-ahmed erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert ia-bot-ahmed mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.ia-bot-ahmed/plugins/` oder `~/.config/ia-bot-ahmed/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie ia-bot-ahmed darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
