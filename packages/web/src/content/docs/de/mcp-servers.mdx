...
Sie können externe Tools zu ia-bot-ahmed hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. ia-bot-ahmed unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/ia-bot-ahmed`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
ia-bot-ahmed übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird ia-bot-ahmed Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von ia-bot-ahmed beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `ia-bot-ahmed mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert ia-bot-ahmed die Token sicher in `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in ia-bot-ahmed verfügbar. Sie können sie also wie jedes andere Tool über die ia-bot-ahmed-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und ia-bot-ahmed mit Ihrem Sentry-Konto zu verbinden.
