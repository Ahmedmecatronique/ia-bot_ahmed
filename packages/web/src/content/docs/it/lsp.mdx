---
title: Server LSP
description: ia-bot-ahmed si integra con i tuoi server LSP.
---

ia-bot-ahmed può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

ia-bot-ahmed include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `IA_BOT_AHMED_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e ia-bot-ahmed apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di ia-bot-ahmed.
