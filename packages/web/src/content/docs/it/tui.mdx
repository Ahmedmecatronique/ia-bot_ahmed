---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di ia-bot-ahmed.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ia-bot-ahmed fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire ia-bot-ahmed avvia la TUI per la directory corrente.
...
Quando usi la TUI di ia-bot-ahmed, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a ia-bot-ahmed. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da ia-bot-ahmed. _Alias_: `/quit`, `/q`
...
Questo è separato da `ia-bot-ahmed.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo ia-bot-ahmed attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `ia-bot-ahmed.default`.
