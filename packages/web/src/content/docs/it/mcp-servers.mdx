...
Puoi aggiungere strumenti esterni a ia-bot-ahmed usando il _Model Context Protocol_, o MCP. ia-bot-ahmed supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/ia-bot-ahmed`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
ia-bot-ahmed gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, ia-bot-ahmed:
...
Se il server richiede autenticazione, ia-bot-ahmed ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `ia-bot-ahmed mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, ia-bot-ahmed memorizzera' i token in modo sicuro in `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in ia-bot-ahmed insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione ia-bot-ahmed come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare ia-bot-ahmed al tuo account Sentry.
