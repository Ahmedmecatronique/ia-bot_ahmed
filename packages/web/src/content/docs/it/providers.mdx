---
title: Provider
description: Usa qualsiasi provider LLM in ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di ia-bot-ahmed.
...
configurazione di ia-bot-ahmed.
...
ia-bot-ahmed Zen e una lista di modelli forniti dal team ia-bot-ahmed, testati e
verificati per funzionare bene con ia-bot-ahmed. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da ia-bot-ahmed Zen.
:::

1. Esegui `/connect` nella TUI, seleziona ia-bot-ahmed e vai su [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/auth).
...
Funziona come qualsiasi altro provider in ia-bot-ahmed ed e totalmente opzionale.
...
Per usare Amazon Bedrock con ia-bot-ahmed:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui ia-bot-ahmed:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `ia-bot-ahmed.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in ia-bot-ahmed non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare ia-bot-ahmed per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché ia-bot-ahmed funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di ia-bot-ahmed.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `ia-bot-ahmed`, Scopes: `api`
...
salvare il token nello storage di auth di ia-bot-ahmed.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
ia-bot-ahmed usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare ia-bot-ahmed
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `ia-bot-ahmed.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `ia-bot-ahmed.json`:
...
Per usare il tuo abbonamento GitHub Copilot con ia-bot-ahmed:
...
Per usare Google Vertex AI con ia-bot-ahmed:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui ia-bot-ahmed.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da ia-bot-ahmed, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) per loggare automaticamente ogni conversazione di ia-bot-ahmed come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di ia-bot-ahmed elencata come sessione separata.
...
Puoi configurare ia-bot-ahmed per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare ia-bot-ahmed per usare modelli locali tramite LM Studio.
...
Puoi configurare ia-bot-ahmed per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per ia-bot-ahmed. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/ia-bot-ahmed) per dettagli.
:::
...
Per usare Ollama Cloud con ia-bot-ahmed:
...
3. Copia la chiave API da usare in ia-bot-ahmed.
...
6. **Importante**: Prima di usare modelli cloud in ia-bot-ahmed, devi scaricare le informazioni del modello localmente:
...
ia-bot-ahmed Zen è una lista di modelli testati e verificati forniti dal team ia-bot-ahmed. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>ia-bot-ahmed Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di ia-bot-ahmed.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di ia-bot-ahmed.
...
5. Puoi anche personalizzarli tramite la configurazione di ia-bot-ahmed. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con ia-bot-ahmed:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di ia-bot-ahmed. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con ia-bot-ahmed. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `ia-bot-ahmed.json` nella directory del progetto:
...
I campi `limit` permettono a ia-bot-ahmed di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `ia-bot-ahmed auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di ia-bot-ahmed e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config ia-bot-ahmed.
