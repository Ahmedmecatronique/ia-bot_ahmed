...
In ia-bot-ahmed esistono due tipi di agenti: agenti primari e subagenti.
...
ia-bot-ahmed include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
ia-bot-ahmed include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

ia-bot-ahmed include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di ia-bot-ahmed, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `ia-bot-ahmed.json`:
...
- Globale: `~/.config/ia-bot-ahmed/agents/`
- Per progetto: `.ia-bot-ahmed/agents/`
...
Se non specifichi una temperature, ia-bot-ahmed usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di ia-bot-ahmed sia per la config specifica del progetto.
...
L'ID modello nella configurazione ia-bot-ahmed usa il formato `provider/model-id`. Per esempio, se stai usando [ia-bot-ahmed Zen](/docs/zen), useresti `ia-bot-ahmed/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `ia-bot-ahmed models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/ia-bot-ahmed).
:::
