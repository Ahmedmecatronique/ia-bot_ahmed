...
description: Usa la configurazione JSON di ia-bot-ahmed.
---

Puoi configurare ia-bot-ahmed usando un file di configurazione JSON.

---

## Formato

ia-bot-ahmed supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/ia-bot-ahmed`) - default dell'organizzazione
2. **Config globale** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `IA_BOT_AHMED_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`ia-bot-ahmed.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.ia-bot-ahmed`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `IA_BOT_AHMED_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.ia-bot-ahmed` e `~/.config/ia-bot-ahmed` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/ia-bot-ahmed`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di ia-bot-ahmed in `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/ia-bot-ahmed/tui.json`.
...
Aggiungi `ia-bot-ahmed.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando ia-bot-ahmed si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `IA_BOT_AHMED_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `IA_BOT_AHMED_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.ia-bot-ahmed` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.ia-bot-ahmed`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json).

La configurazione TUI usa [**`ia-bot-ahmed.app/tui.json`**](https://ia-bot-ahmed.app/tui.json).
...
Usa `IA_BOT_AHMED_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `ia-bot-ahmed.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `ia-bot-ahmed serve` e `ia-bot-ahmed web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server ia-bot-ahmed.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `ia-bot-ahmed.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in ia-bot-ahmed tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, ia-bot-ahmed prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/ia-bot-ahmed/agents/` o `.ia-bot-ahmed/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, ia-bot-ahmed fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`ia-bot-ahmed run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/ia-bot-ahmed/commands/` o `.ia-bot-ahmed/commands/`. [Scopri di piu](/docs/commands).
...
ia-bot-ahmed scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, ia-bot-ahmed **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono ia-bot-ahmed con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.ia-bot-ahmed/plugins/` o `~/.config/ia-bot-ahmed/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare ia-bot-ahmed a usare solo alcuni provider invece di disabilitarli uno per uno.
