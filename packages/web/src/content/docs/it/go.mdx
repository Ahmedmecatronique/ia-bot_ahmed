...
ia-bot-ahmed Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in ia-bot-ahmed. Ti abboni a ia-bot-ahmed Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare ia-bot-ahmed.
...
ia-bot-ahmed Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

ia-bot-ahmed Go funziona come qualsiasi altro provider in ia-bot-ahmed.

1. Accedi a **<a href={console}>ia-bot-ahmed Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `ia-bot-ahmed Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a ia-bot-ahmed Go.
:::
...
ia-bot-ahmed Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua ia-bot-ahmed config
utilizza il formato `ia-bot-ahmed-go/<model-id>`. Ad esempio, per Kimi K2.7 Code, useresti
`ia-bot-ahmed-go/kimi-k2.7-code` nella tua configurazione.
...
Abbiamo creato ia-bot-ahmed Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con ia-bot-ahmed.
