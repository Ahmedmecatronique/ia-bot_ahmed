---
title: Plugin
description: Scrivi plugin per estendere ia-bot-ahmed.
---

I plugin ti permettono di estendere ia-bot-ahmed agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - plugin a livello progetto
- `~/.config/ia-bot-ahmed/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Config globale (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Config di progetto (`ia-bot-ahmed.json`)
3. Directory plugin globale (`~/.config/ia-bot-ahmed/plugins/`)
4. Directory plugin di progetto (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di ia-bot-ahmed per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere ia-bot-ahmed.
...
:::note
Se usi l'app desktop di ia-bot-ahmed, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a ia-bot-ahmed di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a ia-bot-ahmed:
...
L'helper `tool` crea uno strumento personalizzato che ia-bot-ahmed può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in ia-bot-ahmed insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://ia-bot-ahmed.app/docs/sdk) per i dettagli.
