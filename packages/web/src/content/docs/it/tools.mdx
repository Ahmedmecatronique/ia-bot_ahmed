...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. ia-bot-ahmed include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in ia-bot-ahmed.
...
:::note
Questo strumento e' disponibile solo quando `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (oppure `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider ia-bot-ahmed o quando la variabile d'ambiente `IA_BOT_AHMED_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di ia-bot-ahmed:
