---
title: Server
description: Interagisci con il server ia-bot-ahmed via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `ia-bot-ahmed serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client ia-bot-ahmed.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `ia-bot-ahmed.local` |
...
Imposta `IA_BOT_AHMED_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `ia-bot-ahmed`, oppure imposta `IA_BOT_AHMED_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `ia-bot-ahmed serve` sia per `ia-bot-ahmed web`.
...
Quando esegui `ia-bot-ahmed` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server ia-bot-ahmed per interagire con ia-bot-ahmed in modo programmatico.
:::

Questa architettura permette a ia-bot-ahmed di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `ia-bot-ahmed serve` per avviare un server standalone. Se la TUI di ia-bot-ahmed e gia in esecuzione, `ia-bot-ahmed serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di ia-bot-ahmed.
...
Il server ia-bot-ahmed espone le seguenti API.
