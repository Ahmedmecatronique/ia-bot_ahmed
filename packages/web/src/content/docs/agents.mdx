...
There are two types of agents in ia-bot-ahmed; primary agents and subagents.
...
ia-bot-ahmed comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
ia-bot-ahmed comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

ia-bot-ahmed comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into ia-bot-ahmed's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `ia-bot-ahmed.json` config file:
...
- Per-project: `.ia-bot-ahmed/agents/`
...
If no temperature is specified, ia-bot-ahmed uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global ia-bot-ahmed config and the project specific config.
...
The model ID in your ia-bot-ahmed config uses the format `provider/model-id`. For example, if you're using [ia-bot-ahmed Zen](/docs/zen), you would use `ia-bot-ahmed/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `ia-bot-ahmed models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/ia-bot-ahmed).
:::
