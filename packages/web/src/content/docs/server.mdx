---
title: Server
description: Interact with ia-bot-ahmed server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `ia-bot-ahmed serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an ia-bot-ahmed client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `ia-bot-ahmed.local` |
...
Set `IA_BOT_AHMED_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `ia-bot-ahmed`, or set `IA_BOT_AHMED_SERVER_USERNAME` to override it. This applies to both `ia-bot-ahmed serve` and `ia-bot-ahmed web`.
...
When you run `ia-bot-ahmed` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the ia-bot-ahmed server to interact with ia-bot-ahmed programmatically.
:::

This architecture lets ia-bot-ahmed support multiple clients and allows you to interact with ia-bot-ahmed programmatically.

You can run `ia-bot-ahmed serve` to start a standalone server. If you have the
ia-bot-ahmed TUI running, `ia-bot-ahmed serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the ia-bot-ahmed [IDE](/docs/ide) plugins.
...
The ia-bot-ahmed server exposes the following APIs.
