---
title: TUI
description: Using the ia-bot-ahmed terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ia-bot-ahmed provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running ia-bot-ahmed starts the TUI for the current directory.
...
When using the ia-bot-ahmed TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to ia-bot-ahmed. Allows you to select from available providers and add their API keys.
...
Exit ia-bot-ahmed. _Aliases_: `/quit`, `/q`
...
This is separate from `ia-bot-ahmed.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long ia-bot-ahmed waits after the leader key. Defaults to `2000`.
...
Use `IA_BOT_AHMED_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `ia-bot-ahmed.default`.
