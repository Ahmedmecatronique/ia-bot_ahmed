...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي ia-bot-ahmed مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في ia-bot-ahmed.
...
:::note
هذه الأداة متاحة فقط عند ضبط `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (أو `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود ia-bot-ahmed أو عند ضبط متغير البيئة `IA_BOT_AHMED_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل ia-bot-ahmed:
