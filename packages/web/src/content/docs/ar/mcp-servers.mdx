...
يمكنك إضافة أدوات خارجية إلى ia-bot-ahmed باستخدام _Model Context Protocol_، أو MCP. يدعم ia-bot-ahmed خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات ia-bot-ahmed](https://ia-bot-ahmed.app/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/ia-bot-ahmed` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى ia-bot-ahmed تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم ia-bot-ahmed بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك ia-bot-ahmed المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `ia-bot-ahmed mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن ia-bot-ahmed الرموز بشكل آمن في `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل ia-bot-ahmed إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات ia-bot-ahmed مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط ia-bot-ahmed بحساب Sentry الخاص بك.
