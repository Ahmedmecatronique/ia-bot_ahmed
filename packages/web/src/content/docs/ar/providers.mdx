---
title: المزوّدون
description: استخدام أي موفّر LLM في ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم ia-bot-ahmed ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات ia-bot-ahmed.
...
في `~/.local/share/ia-bot-ahmed/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات ia-bot-ahmed.
...
ia-bot-ahmed Zen هي قائمة نماذج يوفّرها فريق ia-bot-ahmed وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع ia-bot-ahmed. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع ia-bot-ahmed Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر ia-bot-ahmed، ثم انتقل إلى [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/auth).
...
يعمل مثل أي مزوّد آخر في ia-bot-ahmed واستخدامه اختياري بالكامل.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق ia-bot-ahmed، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع ia-bot-ahmed.

1. شغّل الأمر `/connect` في TUI، واختر `ia-bot-ahmed Go`، ثم انتقل إلى [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
لاستخدام Amazon Bedrock مع ia-bot-ahmed:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل ia-bot-ahmed:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `ia-bot-ahmed.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل ia-bot-ahmed غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين ia-bot-ahmed لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل ia-bot-ahmed بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات ia-bot-ahmed.
...
تخزين الرمز في مساحة تخزين مصادقة ia-bot-ahmed.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم ia-bot-ahmed نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر ia-bot-ahmed
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`ia-bot-ahmed.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع ia-bot-ahmed:
...
لاستخدام Google Vertex AI مع ia-bot-ahmed:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل ia-bot-ahmed.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر ia-bot-ahmed، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) لتسجيل كل محادثة ia-bot-ahmed تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة ia-bot-ahmed مدرجة كجلسة مستقلة.
...
يمكنك ضبط ia-bot-ahmed لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط ia-bot-ahmed لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط ia-bot-ahmed لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع ia-bot-ahmed. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/ia-bot-ahmed) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع ia-bot-ahmed:
...
3. انسخ مفتاح API لاستخدامه في ia-bot-ahmed.
...
6. **مهم**: قبل استخدام نماذج السحابة في ia-bot-ahmed، يجب سحب معلومات النموذج محليا:
...
ia-bot-ahmed Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق ia-bot-ahmed. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>ia-bot-ahmed Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ ia-bot-ahmed.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات ia-bot-ahmed.
...
5. يمكنك أيضا تخصيصها عبر إعدادات ia-bot-ahmed. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع ia-bot-ahmed:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات ia-bot-ahmed. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع ia-bot-ahmed. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `ia-bot-ahmed.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ ia-bot-ahmed بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `ia-bot-ahmed auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات ia-bot-ahmed وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات ia-bot-ahmed.
