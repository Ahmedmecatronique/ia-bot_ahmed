---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع ia-bot-ahmed.
---

تتيح لك الإضافات توسيع ia-bot-ahmed عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - إضافات على مستوى المشروع
- `~/.config/ia-bot-ahmed/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. إعدادات عامة (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. إعدادات المشروع (`ia-bot-ahmed.json`)
3. دليل الإضافات العام (`~/.config/ia-bot-ahmed/plugins/`)
4. دليل إضافات المشروع (`.ia-bot-ahmed/plugins/`)
...
يشغّل ia-bot-ahmed الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ ia-bot-ahmed للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع ia-bot-ahmed.
...
:::note
إذا كنت تستخدم تطبيق ia-bot-ahmed لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع ia-bot-ahmed من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى ia-bot-ahmed:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ ia-bot-ahmed استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ ia-bot-ahmed إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://ia-bot-ahmed.app/docs/sdk) للتفاصيل.
