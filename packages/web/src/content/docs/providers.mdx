---
title: Providers
description: Using any LLM provider in ia-bot-ahmed.
---

import config from "../../../config.mjs"
export const console = config.console

ia-bot-ahmed uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your ia-bot-ahmed config.
...
in `~/.local/share/ia-bot-ahmed/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your ia-bot-ahmed
config.
...
```json title="ia-bot-ahmed.json" {6}
{
...
ia-bot-ahmed Zen is a list of models provided by the ia-bot-ahmed team that have been
tested and verified to work well with ia-bot-ahmed. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with ia-bot-ahmed Zen.
:::

1. Run the `/connect` command in the TUI, select `ia-bot-ahmed Zen`, and head to [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
It works like any other provider in ia-bot-ahmed and is completely optional to use.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the ia-bot-ahmed team that have been
tested and verified to work well with ia-bot-ahmed.

1. Run the `/connect` command in the TUI, select `ia-bot-ahmed Go`, and head to [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
To use Amazon Bedrock with ia-bot-ahmed:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running ia-bot-ahmed:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY ia-bot-ahmed

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile ia-bot-ahmed

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX ia-bot-ahmed
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `ia-bot-ahmed.json`:
...
ia-bot-ahmed. Anthropic explicitly prohibits this.

Previous versions of ia-bot-ahmed came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in ia-bot-ahmed with zero setup:
...
You can configure ia-bot-ahmed to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="ia-bot-ahmed.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for ia-bot-ahmed to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX ia-bot-ahmed
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX ia-bot-ahmed
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your ia-bot-ahmed config.
...
ia-bot-ahmed supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; ia-bot-ahmed uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize ia-bot-ahmed. Sign in and approve.

   :::note
   ia-bot-ahmed requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, ia-bot-ahmed surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — ia-bot-ahmed forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in ia-bot-ahmed is experimental. Features, configuration, and
behavior may change in future releases.
:::

ia-bot-ahmed integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `ia-bot-ahmed`, Scopes: `api`
...
to store token in ia-bot-ahmed auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
ia-bot-ahmed uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock ia-bot-ahmed
to only use your own GitLab-hosted instance, add the following to your
`ia-bot-ahmed.json` file. It is also recommended to disable session sharing.
...
Customize through `ia-bot-ahmed.json`:
...
When a `duo-workflow-*` model is selected, ia-bot-ahmed will:
...
4. Route tool execution requests through ia-bot-ahmed's permission-gated tool system
...
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "plugin": ["ia-bot-ahmed-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with ia-bot-ahmed:
...
To use Google Vertex AI with ia-bot-ahmed:
...
2. Set the required environment variables:
   ...
   Set them while running ia-bot-ahmed.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id ia-bot-ahmed
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through ia-bot-ahmed, you can always configure it yourself.
...
```jsonc title="~/.config/ia-bot-ahmed/ia-bot-ahmed.jsonc"
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) plugin to automatically log each ia-bot-ahmed conversation as a session in Helicone.

```bash
npm install -g ia-bot-ahmed-helicone-session
```
...
```json title="ia-bot-ahmed.json"
{
  "plugin": ["ia-bot-ahmed-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each ia-bot-ahmed conversation listed as a separate session.
...
You can configure ia-bot-ahmed to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="ia-bot-ahmed.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure ia-bot-ahmed to use local models through LM Studio.

```json title="ia-bot-ahmed.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "lmstudio": {
...
You can configure ia-bot-ahmed to use local models through Ollama.

:::tip
Ollama can automatically configure itself for ia-bot-ahmed. See the [Ollama integration docs](https://docs.ollama.com/integrations/ia-bot-ahmed) for details.
:::

```json title="ia-bot-ahmed.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with ia-bot-ahmed:
...
3. Copy the API key for use in ia-bot-ahmed.
...
6. **Important**: Before using cloud models in ia-bot-ahmed, you must pull the model information locally:
...
ia-bot-ahmed Zen is a list of tested and verified models provided by the ia-bot-ahmed team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>ia-bot-ahmed Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Enter your ia-bot-ahmed API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your ia-bot-ahmed config.
...
5. You can also customize them through your ia-bot-ahmed config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' ia-bot-ahmed
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group ia-bot-ahmed
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with ia-bot-ahmed:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=ia-bot-ahmed&utm_medium=docs&utm_campaign=cortex-provider).

ia-bot-ahmed's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. ia-bot-ahmed captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting ia-bot-ahmed:
...
The model catalog is provided automatically. A minimal `ia-bot-ahmed.json` is all that's needed:
...
You can also customize models through your ia-bot-ahmed config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. ia-bot-ahmed opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
ia-bot-ahmed refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when ia-bot-ahmed is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running ia-bot-ahmed — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. ia-bot-ahmed prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. ia-bot-ahmed polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with ia-bot-ahmed. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in ia-bot-ahmed.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `ia-bot-ahmed.json` file in your project directory:

   ```json title="ia-bot-ahmed.json" ""myprovider"" {5-15}
   {
     "$schema": "https://ia-bot-ahmed.app/config.json",
     "provider": {
       "myprovider": {
...
```json title="ia-bot-ahmed.json" {9,11,17-20}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow ia-bot-ahmed to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `ia-bot-ahmed auth list` to see if the credentials
...
2. For custom providers, check the ia-bot-ahmed config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your ia-bot-ahmed config.
