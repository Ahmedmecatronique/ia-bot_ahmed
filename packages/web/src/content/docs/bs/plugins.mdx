---
title: Dodaci
description: Napišite vlastite dodatke za proširenje ia-bot-ahmed.
---

Dodaci vam omogućavaju da proširite ia-bot-ahmed spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - Dodaci na nivou projekta
- `~/.config/ia-bot-ahmed/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Konfiguracija projekta (`ia-bot-ahmed.json`)
3. Globalni direktorij dodataka (`~/.config/ia-bot-ahmed/plugins/`)
4. Direktorij dodataka projekta (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: ia-bot-ahmed SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje ia-bot-ahmed.
...
Spriječite ia-bot-ahmed da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u ia-bot-ahmed:
...
Pomoćnik `tool` kreira prilagođeni alat koji ia-bot-ahmed može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za ia-bot-ahmed zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://ia-bot-ahmed.app/docs/sdk) za detalje.
