---
title: Server
description: Komunicirajte s ia-bot-ahmed serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `ia-bot-ahmed serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji ia-bot-ahmed klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `ia-bot-ahmed.local` |
...
Postavite `IA_BOT_AHMED_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `ia-bot-ahmed`, ili postavite `IA_BOT_AHMED_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `ia-bot-ahmed serve` i za `ia-bot-ahmed web`.
...
Kada pokrenete `ia-bot-ahmed`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite ia-bot-ahmed server za programsku interakciju sa ia-bot-ahmed-om.
:::

Ova arhitektura omogucava ia-bot-ahmed podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `ia-bot-ahmed serve` da startate standalone server. Ako je ia-bot-ahmed TUI vec pokrenut, `ia-bot-ahmed serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste ia-bot-ahmed [IDE](/docs/ide) pluginovi.
...
ia-bot-ahmed server izlaže sljedece API-je.
