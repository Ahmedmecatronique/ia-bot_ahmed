...
U ia-bot-ahmed mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, ia-bot-ahmed koristi nasu `ia-bot-ahmed` temu.
...
ia-bot-ahmed dolazi sa vise ugradenih tema.
...
- Zele da ia-bot-ahmed odgovara izgledu njihovog terminala
...
ia-bot-ahmed podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/ia-bot-ahmed/themes/*.json` ili `$XDG_CONFIG_HOME/ia-bot-ahmed/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.ia-bot-ahmed/themes/*.json`
4. **Trenutni radni direktorij** - `./.ia-bot-ahmed/themes/*.json`
