---
title: Konfiguracija
description: Korištenje ia-bot-ahmed JSON konfiguracije.
---

Možete konfigurirati ia-bot-ahmed koristeći JSON konfiguracijski fajl.

---

## Format

ia-bot-ahmed podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/ia-bot-ahmed`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`IA_BOT_AHMED_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`ia-bot-ahmed.json` u projektu) - postavke specifične za projekat
5. **`.ia-bot-ahmed` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`IA_BOT_AHMED_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.ia-bot-ahmed` i `~/.config/ia-bot-ahmed` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/ia-bot-ahmed` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu ia-bot-ahmed konfiguraciju u `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/ia-bot-ahmed/tui.json`.
...
Dodajte `ia-bot-ahmed.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se ia-bot-ahmed pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `IA_BOT_AHMED_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `IA_BOT_AHMED_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.ia-bot-ahmed` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.ia-bot-ahmed`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json).

TUI konfiguracija koristi [**`ia-bot-ahmed.app/tui.json`**](https://ia-bot-ahmed.app/tui.json).
...
Koristite `IA_BOT_AHMED_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `ia-bot-ahmed.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `ia-bot-ahmed serve` i `ia-bot-ahmed web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš ia-bot-ahmed server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `ia-bot-ahmed.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj ia-bot-ahmed konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, ia-bot-ahmed pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/ia-bot-ahmed/agents/` ili `.ia-bot-ahmed/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, ia-bot-ahmed će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`ia-bot-ahmed run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/ia-bot-ahmed/commands/` ili `.ia-bot-ahmed/commands/`. [Saznajte više ovdje](/docs/commands).
...
ia-bot-ahmed će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, ia-bot-ahmed **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju ia-bot-ahmed sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.ia-bot-ahmed/plugins/` ili `~/.config/ia-bot-ahmed/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite ia-bot-ahmed da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
