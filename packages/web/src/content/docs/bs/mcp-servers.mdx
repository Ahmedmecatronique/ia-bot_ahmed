...
Možete dodati vanjske alate u ia-bot-ahmed koristeći _Model Context Protocol_, ili MCP. ia-bot-ahmed podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/ia-bot-ahmed`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
ia-bot-ahmed automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, ia-bot-ahmed će:
...
Ako server zahtijeva autentifikaciju, ia-bot-ahmed će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `ia-bot-ahmed mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, ia-bot-ahmed sigurno čuva tokene u `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u ia-bot-ahmed, zajedno s ugrađenim alatima. Možete njima upravljati kroz ia-bot-ahmed konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete ia-bot-ahmed sa vašim Sentry nalogom.
