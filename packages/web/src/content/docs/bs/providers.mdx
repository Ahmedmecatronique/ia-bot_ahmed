---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj ia-bot-ahmed konfiguraciji.
...
u `~/.local/share/ia-bot-ahmed/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem ia-bot-ahmed
config.
...
ia-bot-ahmed Zen je lista modela koje je obezbedio ia-bot-ahmed tim koji su bili
testirano i potvrđeno da dobro radi sa ia-bot-ahmed. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa ia-bot-ahmed Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite ia-bot-ahmed i idite na [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/auth).
...
Radi kao i svaki drugi provajder u ia-bot-ahmed i potpuno je opcionalan za korištenje.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža ia-bot-ahmed tim i koji su testirani i verificirani da dobro rade s ia-bot-ahmed-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `ia-bot-ahmed Go` i idite na [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Da biste koristili Amazon Bedrock s ia-bot-ahmed:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete ia-bot-ahmed:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `ia-bot-ahmed.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u ia-bot-ahmed.
:::
...
Možete konfigurirati ia-bot-ahmed za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi ia-bot-ahmed ispravno radio.
   :::
...
Također možete dodati modele kroz svoju ia-bot-ahmed konfiguraciju.
...
3.  Naziv: `ia-bot-ahmed`, opseg: `api`
...
da pohrani token u ia-bot-ahmed auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
ia-bot-ahmed koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate ia-bot-ahmed
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`ia-bot-ahmed.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `ia-bot-ahmed.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s ia-bot-ahmed:
...
Za korištenje Google Vertex AI s ia-bot-ahmed:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće ia-bot-ahmed.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem ia-bot-ahmeda, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) da automatski prijavite svaki ia-bot-ahmed razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki ia-bot-ahmed razgovor naveden kao zasebna sesija.
...
Možete konfigurirati ia-bot-ahmed za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati ia-bot-ahmed za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati ia-bot-ahmed za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za ia-bot-ahmed. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/ia-bot-ahmed) za detalje.
:::
...
Da biste koristili Ollama Cloud s ia-bot-ahmed:
...
3. Kopirajte API ključ za korištenje u ia-bot-ahmed.
...
6. **Važno**: Prije upotrebe modela oblaka u ia-bot-ahmed, morate lokalno povući informacije o modelu:
...
ia-bot-ahmed Zen je lista testiranih i verifikovanih modela koju je obezbedio ia-bot-ahmed tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>ia-bot-ahmed Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **ia-bot-ahmed Zen**.

```txt
   /connect
```

3. Unesite svoj ia-bot-ahmed API ključ.
...
Također možete dodati dodatne modele putem vaše ia-bot-ahmed konfiguracije.
...
5. Također ih možete prilagoditi putem vaše ia-bot-ahmed konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa ia-bot-ahmedom:
...
Također možete prilagoditi modele kroz svoju ia-bot-ahmed konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s ia-bot-ahmed-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `ia-bot-ahmed.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju ia-bot-ahmed da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `ia-bot-ahmed auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite ia-bot-ahmed konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj ia-bot-ahmed konfiguraciji.
