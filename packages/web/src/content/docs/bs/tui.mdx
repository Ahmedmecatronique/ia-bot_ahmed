---
title: TUI
description: Korištenje korisničkog interfejsa ia-bot-ahmed terminala.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ia-bot-ahmed pruža interaktivni terminalski interfejs ili TUI za rad na vašim projektima sa LLM.

Pokretanje ia-bot-ahmed pokreće TUI za trenutni direktorij.
...
Kada koristite ia-bot-ahmed TUI, možete upisati `/` nakon čega slijedi ime komande da biste brzo izvršili radnje. na primjer:
...
Dodajte provajdera u ia-bot-ahmed. Omogućava vam da odaberete između dostupnih provajdera i dodate njihove API ključeve.
...
Izađite iz ia-bot-ahmed. _Aliases_: `/quit`, `/q`
...
Ovo je odvojeno od `ia-bot-ahmed.json`, koji konfiguriše ponašanje servera/izvršavanja.
...
- `leader_timeout` - Kontroliše koliko dugo ia-bot-ahmed čeka nakon leader key. Podrazumijevano je `2000`.
...
Koristite `IA_BOT_AHMED_TUI_CONFIG` da učitate prilagođenu putanju TUI konfiguracije.

### Attention

Attention omogućava da vas TUI obavijesti kada ia-bot-ahmed čeka odgovor, treba odobrenje dozvole, prijavi grešku sesije ili završi sesiju. Omogućite ga pomoću `attention.enabled`; ugrađeni događaji reproduciraju zvuk kada se dogode. Desktop obavještenja se šalju samo kada prozor terminala nije u fokusu i ne koriste se za subagent događaje.
...
- `sound_pack` - Sound pack ID koji se koristi. Podrazumijevano je `ia-bot-ahmed.default`.
