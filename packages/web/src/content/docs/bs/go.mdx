...
ia-bot-ahmed Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u ia-bot-ahmed-u. Pretplatite se na ia-bot-ahmed Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili ia-bot-ahmed.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa ia-bot-ahmed-om.
:::
...
ia-bot-ahmed Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

ia-bot-ahmed Go radi kao bilo koji drugi provajder u ia-bot-ahmed-u.

1. Prijavite se na **<a href={console}>ia-bot-ahmed Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `ia-bot-ahmed Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na ia-bot-ahmed Go.
:::
...
ia-bot-ahmed Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj ia-bot-ahmed konfiguraciji
koristi format `ia-bot-ahmed-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`ia-bot-ahmed-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo ia-bot-ahmed Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz ia-bot-ahmed koristite i bilo kojeg drugog provajdera.
