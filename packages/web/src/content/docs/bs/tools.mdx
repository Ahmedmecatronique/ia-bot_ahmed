...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. ia-bot-ahmed dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u ia-bot-ahmed.
...
:::note
Ovaj alat je dostupan samo kada je `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (ili `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz ia-bot-ahmed provajdera ili kada je varijabla `IA_BOT_AHMED_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju ia-bot-ahmed:
