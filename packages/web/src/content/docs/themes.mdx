...
With ia-bot-ahmed you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, ia-bot-ahmed uses our own `ia-bot-ahmed` theme.
...
ia-bot-ahmed comes with several built-in themes.
...
- Want ia-bot-ahmed to match their terminal's appearance
...
ia-bot-ahmed supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/ia-bot-ahmed/themes/*.json` or `$XDG_CONFIG_HOME/ia-bot-ahmed/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.ia-bot-ahmed/themes/*.json`
4. **Current working directory** - Defined in `./.ia-bot-ahmed/themes/*.json`
