...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez ia-bot-ahmed.
...
Voici plusieurs modèles qui fonctionnent bien avec ia-bot-ahmed, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration ia-bot-ahmed.

```json title="ia-bot-ahmed.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [ia-bot-ahmed Zen](/docs/zen), vous utiliserez `ia-bot-ahmed/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="ia-bot-ahmed.jsonc" {7-12,19-24}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "openai": {
...
```jsonc title="ia-bot-ahmed.jsonc" {6-21}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "ia-bot-ahmed": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. ia-bot-ahmed est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

ia-bot-ahmed est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="ia-bot-ahmed.jsonc" {7-18}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "provider": {
    "openai": {
...
Lorsque ia-bot-ahmed démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration ia-bot-ahmed.
