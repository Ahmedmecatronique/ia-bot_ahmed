---
title: Configuration
description: Utilisation de la configuration JSON ia-bot-ahmed.
---

Vous pouvez configurer ia-bot-ahmed à l'aide d'un fichier de configuration JSON.

---

## Format

ia-bot-ahmed prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/ia-bot-ahmed`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) - préférences utilisateur
3. **Configuration personnalisée** (`IA_BOT_AHMED_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`ia-bot-ahmed.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.ia-bot-ahmed`** - agents, commandes, plugins
6. **Configuration en ligne** (`IA_BOT_AHMED_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.ia-bot-ahmed` et `~/.config/ia-bot-ahmed` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/ia-bot-ahmed`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale ia-bot-ahmed dans `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/ia-bot-ahmed/tui.json`.
...
Ajoutez `ia-bot-ahmed.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque ia-bot-ahmed démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `IA_BOT_AHMED_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `IA_BOT_AHMED_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.ia-bot-ahmed`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.ia-bot-ahmed`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json).

La configuration TUI utilise [**`ia-bot-ahmed.app/tui.json`**](https://ia-bot-ahmed.app/tui.json).
...
Utilisez `IA_BOT_AHMED_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `ia-bot-ahmed.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `ia-bot-ahmed serve` et `ia-bot-ahmed web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur ia-bot-ahmed.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `ia-bot-ahmed.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration ia-bot-ahmed via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, ia-bot-ahmed essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/ia-bot-ahmed/agents/` ou `.ia-bot-ahmed/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, ia-bot-ahmed reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`ia-bot-ahmed run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/ia-bot-ahmed/commands/` ou `.ia-bot-ahmed/commands/`. [En savoir plus ici](/docs/commands).
...
ia-bot-ahmed téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, ia-bot-ahmed **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent ia-bot-ahmed avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.ia-bot-ahmed/plugins/` ou `~/.config/ia-bot-ahmed/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre ia-bot-ahmed à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
