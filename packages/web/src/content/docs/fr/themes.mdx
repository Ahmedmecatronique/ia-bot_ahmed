...
Avec ia-bot-ahmed, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, ia-bot-ahmed utilise notre propre thème `ia-bot-ahmed`.
...
ia-bot-ahmed est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que ia-bot-ahmed corresponde à l'apparence de leur terminal
...
ia-bot-ahmed prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/ia-bot-ahmed/themes/*.json` ou `$XDG_CONFIG_HOME/ia-bot-ahmed/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.ia-bot-ahmed/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.ia-bot-ahmed/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/ia-bot-ahmed/themes
vim ~/.config/ia-bot-ahmed/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .ia-bot-ahmed/themes
vim .ia-bot-ahmed/themes/my-theme.json
```
...
{
  "$schema": "https://ia-bot-ahmed.app/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
