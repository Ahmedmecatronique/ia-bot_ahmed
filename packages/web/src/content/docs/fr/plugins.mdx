---
title: Plugins
description: Écrivez vos propres plugins pour étendre ia-bot-ahmed.
---

Les plugins vous permettent d'étendre ia-bot-ahmed en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - Plugins au niveau du projet
- `~/.config/ia-bot-ahmed/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Configuration globale (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Configuration du projet (`ia-bot-ahmed.json`)
3. Répertoire global des plugins (`~/.config/ia-bot-ahmed/plugins/`)
4. Répertoire des plugins du projet (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK ia-bot-ahmed pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre ia-bot-ahmed.
...
:::note
Si vous utilisez l'application de bureau ia-bot-ahmed, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez ia-bot-ahmed de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à ia-bot-ahmed :
...
L'assistant `tool` crée un outil personnalisé que ia-bot-ahmed peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour ia-bot-ahmed aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://ia-bot-ahmed.app/docs/sdk) pour plus de détails.
