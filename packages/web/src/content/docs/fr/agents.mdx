...
Il existe deux types d'agents dans ia-bot-ahmed ; agents primaires et sous-agents.
...
ia-bot-ahmed est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
ia-bot-ahmed est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

ia-bot-ahmed est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `ia-bot-ahmed.json` :
...
- Par projet : `.ia-bot-ahmed/agents/`
...
Si aucune température n'est spécifiée, ia-bot-ahmed utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale ia-bot-ahmed et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration ia-bot-ahmed utilise le format `provider/model-id`. Par exemple, si vous utilisez [ia-bot-ahmed Zen](/docs/zen), vous utiliserez `ia-bot-ahmed/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `ia-bot-ahmed models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/ia-bot-ahmed).
:::
