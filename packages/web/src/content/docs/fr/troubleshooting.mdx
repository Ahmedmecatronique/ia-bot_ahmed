...
Pour déboguer les problèmes avec ia-bot-ahmed, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\ia-bot-ahmed\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `ia-bot-ahmed --log-level DEBUG`.

---

## Stockage

ia-bot-ahmed stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/ia-bot-ahmed/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\ia-bot-ahmed`
...
ia-bot-ahmed Desktop exécute un serveur ia-bot-ahmed local (le side-car `ia-bot-ahmed-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `ia-bot-ahmed` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/ia-bot-ahmed/ia-bot-ahmed.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\ia-bot-ahmed\ia-bot-ahmed.jsonc`
...
ia-bot-ahmed peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/ia-bot-ahmed/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\ia-bot-ahmed\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que ia-bot-ahmed puisse le reconstruire.

1. Quittez complètement ia-bot-ahmed Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/ia-bot-ahmed`
- **Linux** : supprimez `~/.cache/ia-bot-ahmed` (ou exécutez `rm -rf ~/.cache/ia-bot-ahmed`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\ia-bot-ahmed`

3. Redémarrez le bureau ia-bot-ahmed.

---

### Résoudre les problèmes de connexion au serveur

ia-bot-ahmed Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `ia-bot-ahmed.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `IA_BOT_AHMED_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `IA_BOT_AHMED_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, ia-bot-ahmed Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de ia-bot-ahmed.

---

### Les notifications ne s'affichent pas

ia-bot-ahmed Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour ia-bot-ahmed dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau ia-bot-ahmed.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application ia-bot-ahmed Desktop) :

- `ia-bot-ahmed.settings.dat` (serveur de bureau par défaut URL)
- `ia-bot-ahmed.global.dat` et `ia-bot-ahmed.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec ia-bot-ahmed :
...
3. Assurez-vous d'avoir la dernière version avec `ia-bot-ahmed upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `ia-bot-ahmed models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\ia-bot-ahmed`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. ia-bot-ahmed installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\ia-bot-ahmed`

2. Redémarrez ia-bot-ahmed pour réinstaller les derniers packages du fournisseur

Cela forcera ia-bot-ahmed à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
ia-bot-ahmed détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
