---
title: SDK
description: Client JS de type sécurisé pour le serveur ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK ia-bot-ahmed JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler ia-bot-ahmed par programme.
...
Créez une instance de ia-bot-ahmed :

```javascript
import { createia-bot-ahmed } from "@ia-bot-ahmed/sdk"

const { client } = await createia-bot-ahmed()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `ia-bot-ahmed.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${ia-bot-ahmed.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de ia-bot-ahmed, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@ia-bot-ahmed/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*ia-bot-ahmed" },
})
