...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. ia-bot-ahmed est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans ia-bot-ahmed.
...
:::note
Cet outil n'est disponible que lorsque `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (ou `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur ia-bot-ahmed ou lorsque la variable d'environnement `IA_BOT_AHMED_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de ia-bot-ahmed :
