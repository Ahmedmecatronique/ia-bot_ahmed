---
title: Formateurs
description: ia-bot-ahmed utilise des formateurs spécifiques au langage.
---

ia-bot-ahmed formate automatiquement les fichiers après leur écriture ou leur modification à l'aide de formateurs spécifiques au langage. Cela garantit que le code généré suit les styles de code de votre projet.

---

## Formateurs intégrés

ia-bot-ahmed est livré avec plusieurs formateurs intégrés pour les langages et frameworks populaires. Vous trouverez ci-dessous une liste des formateurs, des extensions de fichiers prises en charge et des commandes ou options de configuration dont il a besoin.
...
Ainsi, si votre projet a `prettier` dans votre `package.json`, ia-bot-ahmed l'utilisera automatiquement.

---

## Comment ça marche

Lorsque ia-bot-ahmed écrit ou modifie un fichier, il :
...
Vous pouvez personnaliser les formateurs via la section `formatter` de votre configuration ia-bot-ahmed.
...
```json title="ia-bot-ahmed.json" {5}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "formatter": {
    "prettier": {
...
```json title="ia-bot-ahmed.json" {4-14}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "formatter": {
    "prettier": {
