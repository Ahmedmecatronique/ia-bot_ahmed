---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration ia-bot-ahmed.
...
en `~/.local/share/ia-bot-ahmed/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre ia-bot-ahmed
configuration.
...
ia-bot-ahmed Zen est une liste de modèles fournis par l'équipe ia-bot-ahmed qui ont été
testé et vérifié pour fonctionner correctement avec ia-bot-ahmed. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par ia-bot-ahmed Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez ia-bot-ahmed et dirigez-vous vers [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans ia-bot-ahmed et son utilisation est totalement facultative.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe ia-bot-ahmed qui ont été
testé et vérifié pour fonctionner correctement avec ia-bot-ahmed.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `ia-bot-ahmed Go` et rendez-vous sur [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Pour utiliser Amazon Bedrock avec ia-bot-ahmed :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de ia-bot-ahmed :
...
Pour une configuration spécifique au projet ou persistante, utilisez `ia-bot-ahmed.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans ia-bot-ahmed n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer ia-bot-ahmed pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que ia-bot-ahmed fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration ia-bot-ahmed.
...
3.  Nom : `ia-bot-ahmed`, Portées : `api`
...
pour stocker le jeton dans le stockage d'authentification ia-bot-ahmed.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
ia-bot-ahmed utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller ia-bot-ahmed
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `ia-bot-ahmed.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `ia-bot-ahmed.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec ia-bot-ahmed :
...
Pour utiliser Google Vertex AI avec ia-bot-ahmed :
...
Définissez-les lors de l'exécution de ia-bot-ahmed.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via ia-bot-ahmed, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) pour enregistrer automatiquement chaque conversation ia-bot-ahmed en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation ia-bot-ahmed répertoriée comme une session distincte.
...
Vous pouvez configurer ia-bot-ahmed pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer ia-bot-ahmed pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer ia-bot-ahmed pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour ia-bot-ahmed. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/ia-bot-ahmed) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec ia-bot-ahmed :
...
3. Copiez la clé API à utiliser dans ia-bot-ahmed.
...
6. **Important** : Avant d'utiliser des modèles cloud dans ia-bot-ahmed, vous devez extraire les informations du modèle localement :
...
ia-bot-ahmed Zen est une liste de modèles testés et vérifiés fournie par l'équipe ia-bot-ahmed. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>ia-bot-ahmed Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé ia-bot-ahmed API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration ia-bot-ahmed.
...
5. Vous pouvez également les personnaliser via votre configuration ia-bot-ahmed. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec ia-bot-ahmed :
...
Vous pouvez également personnaliser les modèles via votre configuration ia-bot-ahmed. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec ia-bot-ahmed. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `ia-bot-ahmed.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à ia-bot-ahmed de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `ia-bot-ahmed auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration ia-bot-ahmed et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration ia-bot-ahmed.
