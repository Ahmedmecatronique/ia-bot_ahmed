---
title: Sağlayıcılar
description: ia-bot-ahmed'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı ia-bot-ahmed yapılandırmanızda yapılandırın.
...
`~/.local/share/ia-bot-ahmed/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları ia-bot-ahmed'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
ia-bot-ahmed Zen, ia-bot-ahmed ekibi tarafından sağlanan modellerin bir listesidir.
ia-bot-ahmed ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz ia-bot-ahmed Zen ile başlamanızı öneririz.
:::
...
ia-bot-ahmed'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go, ia-bot-ahmed ile iyi çalıştığı test edilmiş ve doğrulanmış, ia-bot-ahmed ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `ia-bot-ahmed Go`'yu seçin ve [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen) adresine gidin.
...
Amazon Bedrock'u ia-bot-ahmed ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   ia-bot-ahmed'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `ia-bot-ahmed.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi ia-bot-ahmed'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
ia-bot-ahmed'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   ia-bot-ahmed'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   ia-bot-ahmed yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
ia-bot-ahmed, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. ia-bot-ahmed'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`ia-bot-ahmed.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`ia-bot-ahmed.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi ia-bot-ahmed'la kullanmak için:
...
Google Vertex AI'yi ia-bot-ahmed ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları ia-bot-ahmed çalıştırılırken ayarlayın.
...
Helicone'un ia-bot-ahmed aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her ia-bot-ahmed görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her ia-bot-ahmed görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
ia-bot-ahmed'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla ia-bot-ahmed'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla ia-bot-ahmed'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini ia-bot-ahmed için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/ia-bot-ahmed)'a bakın.
:::
...
Ollama Cloud'u ia-bot-ahmed ile kullanmak için:
...
3. ia-bot-ahmed'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: ia-bot-ahmed'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
ia-bot-ahmed Zen, ia-bot-ahmed ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>ia-bot-ahmed Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **ia-bot-ahmed Zen**'i arayın.

   ```txt
   /connect
   ```

3. ia-bot-ahmed API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   ia-bot-ahmed yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca ia-bot-ahmed yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi ia-bot-ahmed ile kullanmak için:
...
Modelleri ia-bot-ahmed yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı ia-bot-ahmed ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `ia-bot-ahmed.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları ia-bot-ahmed'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `ia-bot-ahmed auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için ia-bot-ahmed yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, ia-bot-ahmed yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
