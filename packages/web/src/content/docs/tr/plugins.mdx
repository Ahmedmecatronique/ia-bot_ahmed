---
title: Eklentiler
description: ia-bot-ahmed'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek ia-bot-ahmed'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya ia-bot-ahmed'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.ia-bot-ahmed/plugins/` - Proje düzeyinde eklentiler
- `~/.config/ia-bot-ahmed/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/ia-bot-ahmed/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Project config (`ia-bot-ahmed.json`)
3. Global eklenti dizini (`~/.config/ia-bot-ahmed/plugins/`)
4. Proje eklenti dizini (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir ia-bot-ahmed SDK istemcisi.
...
ia-bot-ahmed'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
ia-bot-ahmed masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
ia-bot-ahmed'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca ia-bot-ahmed'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, ia-bot-ahmed'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra ia-bot-ahmed'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://ia-bot-ahmed.app/docs/sdk) bakın.
