...
_Model Bağlam Protokolü_ veya MCP kullanarak ia-bot-ahmed'a harici araçlar ekleyebilirsiniz. ia-bot-ahmed hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/ia-bot-ahmed` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
ia-bot-ahmed, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde ia-bot-ahmed şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa ia-bot-ahmed, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `ia-bot-ahmed mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından ia-bot-ahmed, belirteçleri `~/.local/share/ia-bot-ahmed/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra ia-bot-ahmed'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi ia-bot-ahmed yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve ia-bot-ahmed'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
