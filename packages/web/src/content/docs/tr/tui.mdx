---
title: TUI
description: ia-bot-ahmed terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ia-bot-ahmed, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

ia-bot-ahmed'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
ia-bot-ahmed TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
ia-bot-ahmed'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
ia-bot-ahmed'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `ia-bot-ahmed.json` dosyasından ayrıdır.
...
- `leader_timeout` - ia-bot-ahmed'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
Özel bir TUI yapılandırma yolu yüklemek için `IA_BOT_AHMED_TUI_CONFIG` kullanın.

### Attention

Attention, ia-bot-ahmed bir yanıt beklediğinde, izin onayı gerektiğinde, oturum hatası oluştuğunda veya oturum tamamlandığında TUI'nin sizi ses ve masaüstü bildirimiyle uyarmasını sağlar. `attention.enabled` ile etkinleştirin; yerleşik olaylar gerçekleştiğinde ses çalar. Masaüstü bildirimleri yalnızca terminal penceresi odakta değilken gönderilir ve subagent olayları için kullanılmaz.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `ia-bot-ahmed.default`.
