...
ia-bot-ahmed Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, ia-bot-ahmed'daki diğer sağlayıcılar gibi çalışır. ia-bot-ahmed Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve ia-bot-ahmed'u kullanmak için bunu kullanmanıza gerek yoktur.
...
ia-bot-ahmed Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

ia-bot-ahmed Go, ia-bot-ahmed'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>ia-bot-ahmed Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `ia-bot-ahmed Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye ia-bot-ahmed Go'ya abone olabilir.
:::
...
ia-bot-ahmed Go aşağıdaki limitleri içerir:
...
ia-bot-ahmed yapılandırmanızdaki [model id](/docs/config/#models) formatı `ia-bot-ahmed-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `ia-bot-ahmed-go/kimi-k2.7-code` kullanmalısınız.
...
ia-bot-ahmed Go'yu şu amaçlarla oluşturduk:
...
4. ia-bot-ahmed ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
