...
ia-bot-ahmed ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak ia-bot-ahmed kendi `ia-bot-ahmed` temasını kullanır.
...
ia-bot-ahmed birden fazla yerleşik temayla gelir.
...
- ia-bot-ahmed'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
ia-bot-ahmed, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/ia-bot-ahmed/themes/*.json` veya `$XDG_CONFIG_HOME/ia-bot-ahmed/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.ia-bot-ahmed/themes/*.json`
4. **Geçerli çalışma dizini** - `./.ia-bot-ahmed/themes/*.json`
