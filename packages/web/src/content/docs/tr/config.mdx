---
title: Yapılandırma
description: ia-bot-ahmed JSON yapılandırması kullanır.
---

ia-bot-ahmed'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

ia-bot-ahmed hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
Örneğin, genel ayarınız `theme: "ia-bot-ahmed"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
1. **Uzak yapılandırma** (`.well-known/ia-bot-ahmed`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`IA_BOT_AHMED_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `ia-bot-ahmed.json`) - projeye özgü ayarlar
5. **`.ia-bot-ahmed` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`IA_BOT_AHMED_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.ia-bot-ahmed` ve `~/.config/ia-bot-ahmed` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/ia-bot-ahmed` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global ia-bot-ahmed yapılandırmanızı `~/.config/ia-bot-ahmed/ia-bot-ahmed.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `ia-bot-ahmed.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

ia-bot-ahmed başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
`IA_BOT_AHMED_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`IA_BOT_AHMED_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.ia-bot-ahmed` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.ia-bot-ahmed` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json)'da tanımlanan bir şemaya sahiptir.
...
`ia-bot-ahmed serve` ve `ia-bot-ahmed web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların ia-bot-ahmed sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `ia-bot-ahmed.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
ia-bot-ahmed yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak ia-bot-ahmed, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
ia-bot-ahmed yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/ia-bot-ahmed/agents/` veya `.ia-bot-ahmed/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, ia-bot-ahmed bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`ia-bot-ahmed run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/ia-bot-ahmed/commands/` veya `.ia-bot-ahmed/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
ia-bot-ahmed başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, ia-bot-ahmed açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) ia-bot-ahmed'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.ia-bot-ahmed/plugins/` veya `~/.config/ia-bot-ahmed/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, ia-bot-ahmed'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
