---
title: Sunucu
description: ia-bot-ahmed sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`ia-bot-ahmed serve` komutu, ia-bot-ahmed istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `ia-bot-ahmed.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `IA_BOT_AHMED_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `ia-bot-ahmed` değeridir; değiştirmek isterseniz `IA_BOT_AHMED_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `ia-bot-ahmed serve` hem de `ia-bot-ahmed web` için geçerlidir.
...
`ia-bot-ahmed` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
ia-bot-ahmed ile programatik etkileşim için ia-bot-ahmed sunucusunu kullanın.
:::

Bu mimari, ia-bot-ahmed'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`ia-bot-ahmed serve` ile bağımsız bir sunucu başlatabilirsiniz. ia-bot-ahmed TUI açıksa, `ia-bot-ahmed serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum ia-bot-ahmed [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
ia-bot-ahmed sunucusu aşağıdaki API'leri sunar.
