...
Tools allow the LLM to perform actions in your codebase. ia-bot-ahmed comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in ia-bot-ahmed.
...
:::note
This tool is only available when `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (or `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the ia-bot-ahmed provider or when the `IA_BOT_AHMED_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching ia-bot-ahmed:
