...
Создайте `.ia-bot-ahmed/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию ia-bot-ahmed или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем ia-bot-ahmed [config](/docs/config):
...
- Глобальный: `~/.config/ia-bot-ahmed/commands/`
- Для каждого проекта: `.ia-bot-ahmed/commands/`
...
ia-bot-ahmed включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
