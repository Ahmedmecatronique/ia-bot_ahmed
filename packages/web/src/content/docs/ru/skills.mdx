...
Навыки агента позволяют ia-bot-ahmed обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
ia-bot-ahmed выполняет поиск в следующих местах:

- Конфигурация проекта: `.ia-bot-ahmed/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/ia-bot-ahmed/skills/<name>/SKILL.md`.
...
Для локальных путей проекта ia-bot-ahmed переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.ia-bot-ahmed/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/ia-bot-ahmed/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.ia-bot-ahmed/skills/git-release/SKILL.md` следующим образом:
...
ia-bot-ahmed перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `ia-bot-ahmed.json`:
...
**Для встроенных агентов** (в формате `ia-bot-ahmed.json`):
