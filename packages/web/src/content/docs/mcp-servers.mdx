...
You can add external tools to ia-bot-ahmed using the _Model Context Protocol_, or MCP. ia-bot-ahmed supports both local and remote servers.
...
You can define MCP servers in your [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/ia-bot-ahmed` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
ia-bot-ahmed automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, ia-bot-ahmed will:
...
If the server requires authentication, ia-bot-ahmed will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `ia-bot-ahmed mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, ia-bot-ahmed will store the tokens securely in `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
Your MCPs are available as tools in ia-bot-ahmed, alongside built-in tools. So you can manage them through the ia-bot-ahmed config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect ia-bot-ahmed to your Sentry account.
