---
title: Configuração
description: Usando a configuração JSON do ia-bot-ahmed.
---

Você pode configurar o ia-bot-ahmed usando um arquivo de configuração JSON.

---

## Formato

O ia-bot-ahmed suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/ia-bot-ahmed`) - padrões organizacionais
2. **Configuração global** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) - preferências do usuário
3. **Configuração personalizada** (`IA_BOT_AHMED_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`ia-bot-ahmed.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.ia-bot-ahmed`** - agentes, comandos, plugins
6. **Configuração inline** (`IA_BOT_AHMED_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.ia-bot-ahmed` e `~/.config/ia-bot-ahmed` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/ia-bot-ahmed`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do ia-bot-ahmed em `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/ia-bot-ahmed/tui.json`.
...
Adicione `ia-bot-ahmed.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o ia-bot-ahmed é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `IA_BOT_AHMED_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `IA_BOT_AHMED_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.ia-bot-ahmed`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.ia-bot-ahmed`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json).

A configuração do TUI usa [**`ia-bot-ahmed.app/tui.json`**](https://ia-bot-ahmed.app/tui.json).
...
Use `IA_BOT_AHMED_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `ia-bot-ahmed.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `ia-bot-ahmed serve` e `ia-bot-ahmed web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor ia-bot-ahmed.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `ia-bot-ahmed.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do ia-bot-ahmed através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o ia-bot-ahmed tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/ia-bot-ahmed/agents/` ou `.ia-bot-ahmed/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o ia-bot-ahmed recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`ia-bot-ahmed run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/ia-bot-ahmed/commands/` ou `.ia-bot-ahmed/commands/`. [Saiba mais aqui](/docs/commands).
...
O ia-bot-ahmed fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o ia-bot-ahmed **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o ia-bot-ahmed com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.ia-bot-ahmed/plugins/` ou `~/.config/ia-bot-ahmed/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o ia-bot-ahmed para usar apenas provedores específicos, em vez de desabilitá-los um a um.
