...
As ferramentas permitem que o LLM execute ações em sua base de código. O ia-bot-ahmed vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no ia-bot-ahmed.
...
:::note
Esta ferramenta está disponível apenas quando `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (ou `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor ia-bot-ahmed ou quando a variável de ambiente `IA_BOT_AHMED_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o ia-bot-ahmed:
