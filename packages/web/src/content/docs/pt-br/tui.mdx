---
title: TUI
description: Usando a interface de usuário de terminal ia-bot-ahmed.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O ia-bot-ahmed fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar ia-bot-ahmed inicia o TUI para o diretório atual.
...
Ao usar o TUI do ia-bot-ahmed, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao ia-bot-ahmed. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do ia-bot-ahmed. _Aliases_: `/quit`, `/q`
...
Isso é separado do `ia-bot-ahmed.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o ia-bot-ahmed espera depois da leader key. O padrão é `2000`.
...
Use `IA_BOT_AHMED_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o ia-bot-ahmed espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `ia-bot-ahmed.default`.
