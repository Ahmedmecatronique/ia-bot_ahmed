...
Você pode adicionar ferramentas externas ao ia-bot-ahmed usando o _Model Context Protocol_, ou MCP. O ia-bot-ahmed suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do ia-bot-ahmed](https://ia-bot-ahmed.app/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/ia-bot-ahmed`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O ia-bot-ahmed lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o ia-bot-ahmed irá:
...
Se o servidor requer autenticação, o ia-bot-ahmed solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `ia-bot-ahmed mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o ia-bot-ahmed armazenará os tokens de forma segura em `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no ia-bot-ahmed, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do ia-bot-ahmed como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o ia-bot-ahmed à sua conta do Sentry.
