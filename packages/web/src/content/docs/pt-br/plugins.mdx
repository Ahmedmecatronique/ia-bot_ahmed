---
title: Plugins
description: Escreva seus próprios plugins para estender o ia-bot-ahmed.
---

Plugins permitem que você estenda o ia-bot-ahmed conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - Plugins em nível de projeto
- `~/.config/ia-bot-ahmed/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Configuração global (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Configuração do projeto (`ia-bot-ahmed.json`)
3. Diretório de plugins global (`~/.config/ia-bot-ahmed/plugins/`)
4. Diretório de plugins do projeto (`.ia-bot-ahmed/plugins/`)
...
O ia-bot-ahmed executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do ia-bot-ahmed para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o ia-bot-ahmed.
...
:::note
Se você estiver usando o aplicativo desktop ia-bot-ahmed, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o ia-bot-ahmed de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao ia-bot-ahmed:
...
O helper `tool` cria uma ferramenta personalizada que o ia-bot-ahmed pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o ia-bot-ahmed junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://ia-bot-ahmed.app/docs/sdk) para detalhes.
