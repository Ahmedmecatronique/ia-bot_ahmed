---
title: Servidor
description: Interaja com o servidor ia-bot-ahmed via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `ia-bot-ahmed serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente ia-bot-ahmed pode usar.
...
```bash
ia-bot-ahmed serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `ia-bot-ahmed.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
ia-bot-ahmed serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `IA_BOT_AHMED_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `ia-bot-ahmed`, ou defina `IA_BOT_AHMED_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `ia-bot-ahmed serve` quanto ao `ia-bot-ahmed web`.

```bash
IA_BOT_AHMED_SERVER_PASSWORD=your-password ia-bot-ahmed serve
```
...
Quando você executa `ia-bot-ahmed`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor ia-bot-ahmed para interagir com o ia-bot-ahmed programaticamente.
:::

Essa arquitetura permite que o ia-bot-ahmed suporte múltiplos clientes e permite que você interaja com o ia-bot-ahmed programaticamente.

Você pode executar `ia-bot-ahmed serve` para iniciar um servidor autônomo. Se você tiver o TUI do ia-bot-ahmed em execução, `ia-bot-ahmed serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do ia-bot-ahmed [IDE](/docs/ide).
...
O servidor ia-bot-ahmed expõe as seguintes APIs.
