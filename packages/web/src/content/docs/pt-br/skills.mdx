...
As habilidades do agente permitem que o ia-bot-ahmed descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O ia-bot-ahmed pesquisa nesses locais:

- Configuração do projeto: `.ia-bot-ahmed/skills/<name>/SKILL.md`
- Configuração global: `~/.config/ia-bot-ahmed/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o ia-bot-ahmed sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.ia-bot-ahmed/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/ia-bot-ahmed/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.ia-bot-ahmed/skills/git-release/SKILL.md` assim:
...
O ia-bot-ahmed lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `ia-bot-ahmed.json`:
...
**Para agentes embutidos** (em `ia-bot-ahmed.json`):
