...
O ia-bot-ahmed Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no ia-bot-ahmed. Você assina o ia-bot-ahmed Go e
...
O ia-bot-ahmed Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O ia-bot-ahmed Go funciona como qualquer outro provedor no ia-bot-ahmed.

1. Você entra no **<a href={console}>ia-bot-ahmed Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `ia-bot-ahmed Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o ia-bot-ahmed Go.
:::
...
O ia-bot-ahmed Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do ia-bot-ahmed
usa o formato `ia-bot-ahmed-go/<model-id>`. Por exemplo, para o Kimi K2.7 Code, você usaria
`ia-bot-ahmed-go/kimi-k2.7-code` na sua configuração.
...
Criamos o ia-bot-ahmed Go para:
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o ia-bot-ahmed também.
