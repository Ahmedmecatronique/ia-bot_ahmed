---
title: Provedores
description: Usando qualquer provedor LLM no ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do ia-bot-ahmed.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/ia-bot-ahmed/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do ia-bot-ahmed.
...
ia-bot-ahmed Zen é uma lista de modelos fornecidos pela equipe do ia-bot-ahmed que foram testados e verificados para funcionar bem com o ia-bot-ahmed. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o ia-bot-ahmed Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `ia-bot-ahmed Zen` e acesse [ia-bot-ahmed.app/zen](https://ia-bot-ahmed.app/zen).
...
Funciona como qualquer outro provedor no ia-bot-ahmed e é completamente opcional.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do ia-bot-ahmed que foram
testados e verificados para funcionar bem com o ia-bot-ahmed.

1. Execute o comando `/connect` no TUI, selecione `ia-bot-ahmed Go` e acesse [ia-bot-ahmed.app/zen](https://ia-bot-ahmed.app/zen).
...
Para usar o Amazon Bedrock com o ia-bot-ahmed:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o ia-bot-ahmed:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `ia-bot-ahmed.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no ia-bot-ahmed não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o ia-bot-ahmed para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o ia-bot-ahmed funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do ia-bot-ahmed.
...
2. Escolha seu método de autenticação:
   ...
   3. Nome: `ia-bot-ahmed`, Escopos: `api`
...
armazenar o token no armazenamento de autenticação do ia-bot-ahmed.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
ia-bot-ahmed usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o ia-bot-ahmed
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `ia-bot-ahmed.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `ia-bot-ahmed.json`:
...
Para usar sua assinatura do GitHub Copilot com o ia-bot-ahmed:
...
Para usar o Google Vertex AI com o ia-bot-ahmed:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o ia-bot-ahmed.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do ia-bot-ahmed, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) para registrar automaticamente cada conversa do ia-bot-ahmed como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do ia-bot-ahmed listada como uma sessão separada.
...
Você pode configurar o ia-bot-ahmed para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o ia-bot-ahmed para usar modelos locais através do LM Studio.
...
Você pode configurar o ia-bot-ahmed para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o ia-bot-ahmed. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/ia-bot-ahmed) para detalhes.
:::
...
Para usar o Ollama Cloud com o ia-bot-ahmed:
...
3. Copie a chave da API para uso no ia-bot-ahmed.
...
6. **Importante**: Antes de usar modelos em nuvem no ia-bot-ahmed, você deve puxar as informações do modelo localmente:
...
ia-bot-ahmed Zen é uma lista de modelos testados e verificados fornecidos pela equipe do ia-bot-ahmed. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>ia-bot-ahmed Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do ia-bot-ahmed.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do ia-bot-ahmed.
...
5. Você também pode personalizá-los através da sua configuração do ia-bot-ahmed. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o ia-bot-ahmed:
...
Você também pode personalizar modelos através da sua configuração do ia-bot-ahmed. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o ia-bot-ahmed. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no ia-bot-ahmed.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `ia-bot-ahmed.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o ia-bot-ahmed entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `ia-bot-ahmed auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do ia-bot-ahmed e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do ia-bot-ahmed.
