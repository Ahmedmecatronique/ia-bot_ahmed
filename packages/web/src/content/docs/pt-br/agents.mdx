...
Existem dois tipos de agentes no ia-bot-ahmed; agentes primários e subagentes.
...
ia-bot-ahmed vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
ia-bot-ahmed vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

ia-bot-ahmed vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do ia-bot-ahmed, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `ia-bot-ahmed.json`:
...
- Global: `~/.config/ia-bot-ahmed/agents/`
- Por projeto: `.ia-bot-ahmed/agents/`

```markdown title="~/.config/ia-bot-ahmed/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o ia-bot-ahmed usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do ia-bot-ahmed quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do ia-bot-ahmed usa o formato `provider/model-id`. Por exemplo, se você estiver usando [ia-bot-ahmed Zen](/docs/zen), você usaria `ia-bot-ahmed/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="ia-bot-ahmed.json" {3-6,9-12}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "tools": {
    "write": true,
...
```json title="ia-bot-ahmed.json" {3-5,8-10}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "permission": {
    "edit": "deny"
...
```json title="ia-bot-ahmed.json" {8}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "agent": {
    "build": {
...
```json title="ia-bot-ahmed.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `ia-bot-ahmed models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/ia-bot-ahmed).
:::
...
```markdown title="~/.config/ia-bot-ahmed/agents/docs-writer.md"
---
...
```markdown title="~/.config/ia-bot-ahmed/agents/security-auditor.md"
---
