...
Com o ia-bot-ahmed, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o ia-bot-ahmed usa nosso próprio tema `ia-bot-ahmed`.
...
O ia-bot-ahmed vem com vários temas embutidos.
...
- Querem que o ia-bot-ahmed corresponda à aparência do seu terminal
...
O ia-bot-ahmed suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/ia-bot-ahmed/themes/*.json` ou `$XDG_CONFIG_HOME/ia-bot-ahmed/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.ia-bot-ahmed/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.ia-bot-ahmed/themes/*.json`
