...
Możesz dodać dodatkowe informacje do ia-bot-ahmed za pomocą _Model Context Protocol_, w skrócie MCP. ia-bot-ahmed obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/ia-bot-ahmed`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
ia-bot-ahmed automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, ia-bot-ahmed:
...
Jeśli serwer wymaga uwierzytelnienia, ia-bot-ahmed poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `ia-bot-ahmed mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji ia-bot-ahmed bezpieczne przechowa tokeny w `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w ia-bot-ahmed, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie ia-bot-ahmed, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć ia-bot-ahmed z kontem Sentry.
