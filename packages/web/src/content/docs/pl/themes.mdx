...
Dzięki ia-bot-ahmed możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie ia-bot-ahmed używa naszego własnego motywu `ia-bot-ahmed`.
...
ia-bot-ahmed ma kilka wbudowanych motywów.
...
- Chcą, aby ia-bot-ahmed pasował do wyglądu ich terminala
...
ia-bot-ahmed obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/ia-bot-ahmed/themes/*.json` lub `$XDG_CONFIG_HOME/ia-bot-ahmed/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.ia-bot-ahmed/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.ia-bot-ahmed/themes/*.json`
