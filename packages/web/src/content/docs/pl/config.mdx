---
title: Konfiguracja
description: Ustawienia ia-bot-ahmed w pliku JSON.
---

Możesz dostosować ia-bot-ahmed za pomocą pliku konfiguracyjnego JSON.

---

## Format

ia-bot-ahmed obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "ia-bot-ahmed"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/ia-bot-ahmed`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `IA_BOT_AHMED_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`ia-bot-ahmed.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.ia-bot-ahmed`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `IA_BOT_AHMED_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.ia-bot-ahmed` i `~/.config/ia-bot-ahmed` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/ia-bot-ahmed`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację ia-bot-ahmed w `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/ia-bot-ahmed/tui.json`.
...
Dodaj `ia-bot-ahmed.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy ia-bot-ahmed się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `IA_BOT_AHMED_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `IA_BOT_AHMED_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.ia-bot-ahmed` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.ia-bot-ahmed`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json).
...
Użyj `IA_BOT_AHMED_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `ia-bot-ahmed.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `ia-bot-ahmed serve` i `ia-bot-ahmed web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer ia-bot-ahmed.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `ia-bot-ahmed.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie ia-bot-ahmed wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/ia-bot-ahmed/agents/` lub `.ia-bot-ahmed/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, ia-bot-ahmed powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`ia-bot-ahmed run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/ia-bot-ahmed/commands/` lub `.ia-bot-ahmed/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
ia-bot-ahmed może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy ia-bot-ahmed nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie ia-bot-ahmed **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają ia-bot-ahmed o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.ia-bot-ahmed/plugins/` lub `~/.config/ia-bot-ahmed/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie ia-bot-ahmed tylko do określonych dostawców.
