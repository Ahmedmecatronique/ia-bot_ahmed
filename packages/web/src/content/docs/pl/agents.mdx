...
W ia-bot-ahmed obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
ia-bot-ahmed zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
ia-bot-ahmed ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

ia-bot-ahmed ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej ia-bot-ahmed, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `ia-bot-ahmed.json`:
...
- Globalnie: `~/.config/ia-bot-ahmed/agents/`
- W projekcie: `.ia-bot-ahmed/agents/`
...
Jeśli nie zostanie skonfigurowana, ia-bot-ahmed używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji ia-bot-ahmed, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji ia-bot-ahmed w `provider/model-id`. Na przykład, [ia-bot-ahmed Zen](/docs/zen), użyj `ia-bot-ahmed/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `ia-bot-ahmed models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/ia-bot-ahmed).
:::
