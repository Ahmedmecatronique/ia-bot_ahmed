...
Umiejętności agenta pozwalają ia-bot-ahmed odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
ia-bot-ahmed przeszukuje te lokalizacje:

- Project config: `.ia-bot-ahmed/skills/<name>/SKILL.md`
- Global config: `~/.config/ia-bot-ahmed/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu ia-bot-ahmed przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.ia-bot-ahmed/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/ia-bot-ahmed/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.ia-bot-ahmed/skills/git-release/SKILL.md` w ten sposób:
...
ia-bot-ahmed wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `ia-bot-ahmed.json`:
...
**Dla agentów wbudowanych** (w `ia-bot-ahmed.json`):
