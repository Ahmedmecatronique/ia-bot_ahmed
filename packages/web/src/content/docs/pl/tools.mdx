...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. ia-bot-ahmed posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w ia-bot-ahmed.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `IA_BOT_AHMED_EXPERIMENTAL_LSP_TOOL=true` (lub `IA_BOT_AHMED_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy ia-bot-ahmed lub gdy zmienna środowiskowa `IA_BOT_AHMED_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu ia-bot-ahmed:
