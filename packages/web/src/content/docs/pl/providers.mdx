---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji ia-bot-ahmed.
...
w `~/.local/share/ia-bot-ahmed/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim ia-bot-ahmed
config.
...
ia-bot-ahmed Zen to lista modeli dostarczonych przez zespół ia-bot-ahmed, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z ia-bot-ahmed. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od ia-bot-ahmed Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `ia-bot-ahmed Zen` i przejdź do [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Działa jak każdy inny dostawca w ia-bot-ahmed i jest całkowicie opcjonalny w użyciu.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół ia-bot-ahmed, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z ia-bot-ahmed.

1. Uruchom polecenie `/connect` w TUI, wybierz `ia-bot-ahmed Go` i przejdź do [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Aby używać Amazon Bedrock z ia-bot-ahmed:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania ia-bot-ahmed:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `ia-bot-ahmed.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w ia-bot-ahmed nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować ia-bot-ahmed do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod ia-bot-ahmed działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji ia-bot-ahmed.
...
ia-bot-ahmed używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować ia-bot-ahmed
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`ia-bot-ahmed.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem ia-bot-ahmed:
...
Aby używać Google Vertex AI z ia-bot-ahmed:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania ia-bot-ahmed.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą ia-bot-ahmed, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session), aby automatycznie rejestrować każdą konwersację ia-bot-ahmed jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja ia-bot-ahmed będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować ia-bot-ahmed tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować ia-bot-ahmed tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować ia-bot-ahmed tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem ia-bot-ahmed. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/ia-bot-ahmed).
:::
...
Aby korzystać z Ollama Cloud z ia-bot-ahmed:
...
3. Skopiuj klucz API do użycia w ia-bot-ahmed.
...
6. **Ważne**: Przed użyciem modeli chmurowych w ia-bot-ahmed musisz pobrać informacje o modelu lokalnie:
...
ia-bot-ahmed Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół ia-bot-ahmed. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>ia-bot-ahmed Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API ia-bot-ahmed.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji ia-bot-ahmed.
...
5. Możesz je także dostosować za pomocą konfiguracji ia-bot-ahmed. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z ia-bot-ahmed:
...
Możesz także dostosować modele za pomocą konfiguracji ia-bot-ahmed. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z ia-bot-ahmed. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `ia-bot-ahmed.json` w katalogu projektu:
...
Pola `limit` pozwalają ia-bot-ahmed zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `ia-bot-ahmed auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację ia-bot-ahmed i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji ia-bot-ahmed.
