---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć ia-bot-ahmed.
---

Wtyczki umożliwiające wyłączenie ia-bot-ahmed poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania ia-bot-ahmed.
...
- `.ia-bot-ahmed/plugins/` - Wtyczki na poziomie projektu
- `~/.config/ia-bot-ahmed/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Konfiguracja projektu (`ia-bot-ahmed.json`)
3. Globalny katalog wtyczek (`~/.config/ia-bot-ahmed/plugins/`)
4. Katalog wtyczek projektu (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z ia-bot-ahmed do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do ia-bot-ahmed.
...
:::note
Jeśli korzystasz z aplikacji komputerowej ia-bot-ahmed, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj ia-bot-ahmed czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do ia-bot-ahmed:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod ia-bot-ahmed. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://ia-bot-ahmed.app/docs/sdk).
