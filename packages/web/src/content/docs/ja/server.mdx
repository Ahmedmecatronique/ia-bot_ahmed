---
title: サーバー
description: HTTP 経由で ia-bot-ahmed サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`ia-bot-ahmed serve` コマンドは、ia-bot-ahmed クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `ia-bot-ahmed.local` |
...
HTTP 基本認証でサーバーを保護するには、`IA_BOT_AHMED_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `ia-bot-ahmed` になるか、`IA_BOT_AHMED_SERVER_USERNAME` を設定してオーバーライドします。これは、`ia-bot-ahmed serve` と `ia-bot-ahmed web` の両方に当てはまります。
...
`ia-bot-ahmed` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
ia-bot-ahmed サーバーを使用して、プログラムで ia-bot-ahmed と対話します。
:::

`ia-bot-ahmed serve` を実行してスタンドアロンサーバーを起動できます。すでに ia-bot-ahmed TUI を実行している場合でも、`ia-bot-ahmed serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、ia-bot-ahmed [IDE](/docs/ide) プラグインによって使用されます。
...
ia-bot-ahmed サーバーは次の API を公開します。
