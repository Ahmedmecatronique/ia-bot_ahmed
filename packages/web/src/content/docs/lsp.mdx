---
title: LSP Servers
description: ia-bot-ahmed integrates with your LSP servers.
---

ia-bot-ahmed can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

ia-bot-ahmed comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `IA_BOT_AHMED_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and ia-bot-ahmed opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your ia-bot-ahmed config.
...
```json title="ia-bot-ahmed.json" {5-8}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "lsp": {
    "rust": {
...
```json title="ia-bot-ahmed.json" {5-13}
{
  "$schema": "https://ia-bot-ahmed.app/config.json",
  "lsp": {
    "custom-lsp": {
