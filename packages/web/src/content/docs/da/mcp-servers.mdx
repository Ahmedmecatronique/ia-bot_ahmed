...
Du kan tilføje eksterne værktøjer til ia-bot-ahmed ved hjælp af _Model Context Protocol_ eller MCP. ia-bot-ahmed understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [ia-bot-ahmed Config](https://ia-bot-ahmed.app/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/ia-bot-ahmed`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
ia-bot-ahmed håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil ia-bot-ahmed:
...
Hvis serveren kræver godkendelse, vil ia-bot-ahmed bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `ia-bot-ahmed mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil ia-bot-ahmed opbevare tokens sikkert i `~/.local/share/ia-bot-ahmed/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem ia-bot-ahmed-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde ia-bot-ahmed til din Sentry-konto.
