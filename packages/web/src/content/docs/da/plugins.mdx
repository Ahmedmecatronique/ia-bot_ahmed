---
title: Plugins
description: Skriv dine egne plugins for at udvide ia-bot-ahmed.
---

Plugins giver dig mulighed for at udvide ia-bot-ahmed ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre ia-bot-ahmeds standardadfærd.
...
- `.ia-bot-ahmed/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/ia-bot-ahmed/node_modules/`.
...
1. Global konfiguration (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`)
2. Projektkonfiguration (`ia-bot-ahmed.json`)
3. Global plugin-mappe (`~/.config/ia-bot-ahmed/plugins/`)
4. Projekt plugin bibliotek (`.ia-bot-ahmed/plugins/`)
...
ia-bot-ahmed kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En ia-bot-ahmed SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide ia-bot-ahmed.
...
:::note
Hvis du bruger ia-bot-ahmed desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå ia-bot-ahmed i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til ia-bot-ahmed:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som ia-bot-ahmed kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for ia-bot-ahmed sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://ia-bot-ahmed.app/docs/sdk) for detaljer.
