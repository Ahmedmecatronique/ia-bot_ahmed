---
title: Konfiguration
description: Ved at bruge ia-bot-ahmed JSON konfig.
---

Du kan konfigurere ia-bot-ahmed ved hjælp af en JSON-konfigurationsfil.

---

## Format

ia-bot-ahmed understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/ia-bot-ahmed`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/ia-bot-ahmed/ia-bot-ahmed.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`IA_BOT_AHMED_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`ia-bot-ahmed.json` i projekt) - projektspecifikke indstillinger
5. **`.ia-bot-ahmed` mapper** - agent, kommandoer, plugins
6. **Inline config** (`IA_BOT_AHMED_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.ia-bot-ahmed` og `~/.config/ia-bot-ahmed` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/ia-bot-ahmed`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale ia-bot-ahmed-konfiguration i `~/.config/ia-bot-ahmed/ia-bot-ahmed.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `ia-bot-ahmed.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når ia-bot-ahmed starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `IA_BOT_AHMED_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `IA_BOT_AHMED_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.ia-bot-ahmed` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.ia-bot-ahmed` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`ia-bot-ahmed.app/config.json`**](https://ia-bot-ahmed.app/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `ia-bot-ahmed serve` og `ia-bot-ahmed web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din ia-bot-ahmed-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `ia-bot-ahmed.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din ia-bot-ahmed-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger ia-bot-ahmed at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din ia-bot-ahmed-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/ia-bot-ahmed/agents/` eller `.ia-bot-ahmed/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil ia-bot-ahmed falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`ia-bot-ahmed run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/ia-bot-ahmed/commands/` eller `.ia-bot-ahmed/commands/`. [Learn more here](/docs/commands).
...
ia-bot-ahmed vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard ia-bot-ahmed **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide ia-bot-ahmed med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.ia-bot-ahmed/plugins/` eller `~/.config/ia-bot-ahmed/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse ia-bot-ahmed til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
