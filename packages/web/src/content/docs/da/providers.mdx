---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i ia-bot-ahmed.
---

import config from "../../../../config.mjs"
export const console = config.console

ia-bot-ahmed bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i ia-bot-ahmed-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/ia-bot-ahmed/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i ia-bot-ahmed-konfigurationen.
...
ia-bot-ahmed Zen er en liste over modeller leveret af ia-bot-ahmed-teamet, som er blevet testet og verificeret til at fungere godt med ia-bot-ahmed. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med ia-bot-ahmed Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg ia-bot-ahmed og gå til [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/auth).
...
Det fungerer som alle andre udbydere i ia-bot-ahmed og er helt valgfrit at bruge.

---

## ia-bot-ahmed Go

ia-bot-ahmed Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af ia-bot-ahmed-teamet, som er testet og verificeret til at fungere godt med ia-bot-ahmed.

1. Kør kommandoen `/connect` i TUI, vælg `ia-bot-ahmed Go`, og gå til [ia-bot-ahmed.app/auth](https://ia-bot-ahmed.app/zen).
...
Sådan bruger du Amazon Bedrock med ia-bot-ahmed:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører ia-bot-ahmed:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `ia-bot-ahmed.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i ia-bot-ahmed understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere ia-bot-ahmed til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at ia-bot-ahmed skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem ia-bot-ahmed-konfigurationen.
...
2. Vælg din godkendelsesmetode:
   ...
   3. Navn: `ia-bot-ahmed`, omfang: `api`
...
gemme tokenet i ia-bot-ahmed auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
ia-bot-ahmed bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse ia-bot-ahmed
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`ia-bot-ahmed.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Tilpas gennem `ia-bot-ahmed.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med ia-bot-ahmed:
...
Sådan bruger du Google Vertex AI med ia-bot-ahmed:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører ia-bot-ahmed.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem ia-bot-ahmed, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [ia-bot-ahmed-helicone-session](https://github.com/H2Shami/ia-bot-ahmed-helicone-session) plugin for automatisk at logge hver ia-bot-ahmed-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver ia-bot-ahmed-samtale opført som en separat session.
...
Du kan konfigurere ia-bot-ahmed til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere ia-bot-ahmed til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere ia-bot-ahmed til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for ia-bot-ahmed. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/ia-bot-ahmed) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med ia-bot-ahmed:
...
3. Kopier API-nøglen til brug i ia-bot-ahmed.
...
6. **Vigtigt**: Før du bruger skymodeller i ia-bot-ahmed, skal du hente modelinformationen lokalt:
...
ia-bot-ahmed Zen er en liste over testede og verificerede modeller leveret af ia-bot-ahmed-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>ia-bot-ahmed Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **ia-bot-ahmed Zen**.

   ```txt
   /connect
   ```

3. Indtast ia-bot-ahmed API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem ia-bot-ahmed-konfigurationen.
...
5. Du kan også tilpasse dem gennem ia-bot-ahmed-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med ia-bot-ahmed:
...
Du kan også tilpasse modeller gennem ia-bot-ahmed-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med ia-bot-ahmed. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `ia-bot-ahmed.json`-filen i projektkataloget:
...
`limit`-felterne lader ia-bot-ahmed forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `ia-bot-ahmed auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek ia-bot-ahmed-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i ia-bot-ahmed-konfigurationen.
