---
title: Policies
description: Control which configured resources ia-bot-ahmed may use.
---

Policies control whether ia-bot-ahmed may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `ia-bot-ahmed.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether ia-bot-ahmed may use a resource such as an LLM provider.
...
ia-bot-ahmed currently supports one policy action:
