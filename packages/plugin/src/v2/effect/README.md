# ia-bot-ahmed V2 Plugin API

> Design proposal. The API shown here is the intended V2 model and is not fully implemented yet.

This document explains how ia-bot-ahmed V2 plugins contribute agents, commands, skills, integrations, providers, and models without importing `@ia-bot-ahmed/core`.

The design has four goals:

- Internal and external plugins use the same API.
- Plugin values use generated `@ia-bot-ahmed/sdk` types.
...
```ts
import { defineEffectPlugin } from "@ia-bot-ahmed/plugin/v2/effect"
...
When the plugin unloads, ia-bot-ahmed rebuilds the agent registry without this transform. The reviewer disappears automatically.
...
return {
  ...
  "aisdk.sdk": Effect.fn(function* (event) {
    // Runs when ia-bot-ahmed needs an AI SDK provider.
  }),

  "aisdk.language": Effect.fn(function* (event) {
    // Runs when ia-bot-ahmed selects a language model implementation.
  }),
}
```
...
ia-bot-ahmed's default distribution uses an opinionated order.
...
When a plugin is added, ia-bot-ahmed invalidates every domain for which it returned a transform.

When a plugin is removed, ia-bot-ahmed removes its hooks and invalidates those domains. Rebuilding from base state automatically removes the plugin's prior mutations.

When a plugin is replaced, ia-bot-ahmed swaps its hooks, preserves the intended plugin order, and invalidates the affected domains.
