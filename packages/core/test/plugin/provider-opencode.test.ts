...
describe("ia-bot-ahmedPlugin", () => {
  ...
  it.effect("uses IA_BOT_AHMED_API_KEY as credentials", () =>
    withEnv({ IA_BOT_AHMED_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = new ProviderV2.Info({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ IA_BOT_AHMED_API_KEY: undefined, CUSTOM_IA_BOT_AHMED_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        const integrations = yield* Integration.Service
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("ia-bot-ahmed"),
            method: { type: "env", names: ["CUSTOM_IA_BOT_AHMED_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = new ProviderV2.Info({
