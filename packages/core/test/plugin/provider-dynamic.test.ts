...
import { AISDK } from "@ia-bot-ahmed/core/aisdk"
...
import { DynamicProviderPlugin } from "@ia-bot-ahmed/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "ia-bot-ahmed-provider-dynamic-"))
