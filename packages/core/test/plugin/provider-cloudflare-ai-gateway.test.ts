...
import { CloudflareAIGatewayPlugin } from "@ia-bot-ahmed/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              ...
              metadata: { invoked_by: "test", project: "ia-bot-ahmed" },
              cacheTtl: 300,
              cacheKey: "cache-key",
              skipCache: true,
              collectLog: false,
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("ia-bot-ahmed/"),
            },
          },
        })
      }),
    ...
    withEnv(cloudflareEnv(), () =>
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              name: "cloudflare-ai-gateway",
              headers: {
                "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "ia-bot-ahmed" }),
              },
            },
          },
          {},
        ...
        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "ia-bot-ahmed" },
        })
      }),
    ),
  )
