...
import { ModelsDevPlugin } from "@ia-bot-ahmed/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.IA_BOT_AHMED_MODELS_PATH,
          disabled: Flag.IA_BOT_AHMED_DISABLE_MODELS_FETCH,
        }
        Flag.IA_BOT_AHMED_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.IA_BOT_AHMED_MODELS_PATH = previous.path
          Flag.IA_BOT_AHMED_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
