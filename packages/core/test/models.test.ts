...
// test/preload.ts pins IA_BOT_AHMED_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.IA_BOT_AHMED_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.IA_BOT_AHMED_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.IA_BOT_AHMED_MODELS_PATH = undefined
  Flag.IA_BOT_AHMED_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.IA_BOT_AHMED_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.IA_BOT_AHMED_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.IA_BOT_AHMED_DISABLE_MODELS_FETCH = false
        }),
        () =>
          provided(
            state,
