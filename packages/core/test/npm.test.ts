...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@ia-bot-ahmed/acme")).toBe("@ia-bot-ahmed/acme")
    expect(Npm.sanitize("@ia-bot-ahmed/acme@1.0.0")).toBe("@ia-bot-ahmed/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/ia-bot-ahmed/acme.git"
    const expected = win ? "acme@git+https_//github.com/ia-bot-ahmed/acme.git" : spec
