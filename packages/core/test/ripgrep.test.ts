...
import { RelativePath } from "@ia-bot-ahmed/core/schema"
    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".ia-bot-ahmed/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".ia-bot-ahmed/config"))
