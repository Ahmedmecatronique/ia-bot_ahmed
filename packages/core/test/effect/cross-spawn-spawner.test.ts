...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ia-bot-ahmed-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["IA_BOT_AHMED_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { IA_BOT_AHMED_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("IA_BOT_AHMED_TEST_SHELL=ok")
      }),
    )
