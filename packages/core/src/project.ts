...
export class Service extends Context.Service<Service, Interface>()("@ia-bot-ahmed/ProjectV2") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "ia-bot-ahmed")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "ia-bot-ahmed"), input.id).pipe(Effect.ignore)
    })
