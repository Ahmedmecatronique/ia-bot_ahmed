...
export function fileLogger(file = path.join(Global.Path.log, "ia-bot-ahmed.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.IA_BOT_AHMED_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.IA_BOT_AHMED_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
