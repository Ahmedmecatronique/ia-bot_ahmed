...
export class Service extends Context.Service<Service, Interface>()("@ia-bot-ahmed/v2/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "ia-bot-ahmed.json", "ia-bot-ahmed.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".ia-bot-ahmed", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".ia-bot-ahmed")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".ia-bot-ahmed").toReversed()
    ...
    // global config, project files, then `.ia-bot-ahmed` files.
