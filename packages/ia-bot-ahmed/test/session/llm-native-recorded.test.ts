...
import { LLMEvent, LLMResponse } from "@ia-bot-ahmed/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@ia-bot-ahmed/llm/route"
import { Env } from "@/env"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.IA_BOT_AHMED_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["ia-bot-ahmed", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("IA_BOT_AHMED_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("IA_BOT_AHMED_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["ia-bot-ahmed", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["ia-bot-ahmed", "zen", "native", "tool-loop"],
    ...
    config: (model) =>
      providerConfig({
        ...
        api: zenURL(process.env.IA_BOT_AHMED_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.IA_BOT_AHMED_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.IA_BOT_AHMED_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["ia-bot-ahmed", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("IA_BOT_AHMED_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("IA_BOT_AHMED_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("IA_BOT_AHMED_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set IA_BOT_AHMED_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
