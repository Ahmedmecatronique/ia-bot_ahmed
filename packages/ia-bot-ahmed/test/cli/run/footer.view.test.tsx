...
import { ia-bot-ahmedKeymapProvider, registeria-bot-ahmedKeymap } from "@ia-bot-ahmed/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registeria-bot-ahmedKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registeria-bot-ahmedKeymap(keymap, renderer, tuiConfig)
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registeria-bot-ahmedKeymap(keymap, renderer, tuiConfig)
