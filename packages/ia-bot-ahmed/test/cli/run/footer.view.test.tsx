...
import type { QuestionRequest } from "@ia-bot-ahmed/sdk/v2"
import { IaBotAhmedKeymapProvider, registerIaBotAhmedKeymap } from "@ia-bot-ahmed/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerIaBotAhmedKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerIaBotAhmedKeymap(keymap, renderer, tuiConfig)
  ...
  try {
    ...
    expect(frame).toContain("a-model-name-long-enough-to-force-responsive-truncation")
    expect(frame).toContain("subagents Â· ctrl+p cmd")
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "ia-bot-ahmed", modelID: "gpt-5" },
  ...
  try {
    ...
    expect(frame).toContain("xhigh Â· ctrl+x down subagents Â· ctrl+p cmd")
...
test("direct footer hides the subagent hint when only completed subagents remain", async () => {
  ...
  try {
    ...
    expect(frame).toContain("GPT-5")
    expect(frame).toContain("xhigh Â· ctrl+p cmd")
...
test("direct footer shows full usage metadata when room is available", async () => {
  ...
  try {
    ...
    expect(frame).toContain("159.6K (16%) Â· $4.23")
  } finally {
    app.cleanup()
  }
})
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerIaBotAhmedKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "ia-bot-ahmed", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("ia-bot-ahmed")
    expect(frame).toContain("GPT-5")
