  ...
  cliIt.live(
    ...
    ({ IaBotAhmed }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("ia-bot-ahmed")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ IaBotAhmed }) =>
      Effect.gen(function* () {
        ...
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "ia-bot-ahmed-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.IA_BOT_AHMED_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
