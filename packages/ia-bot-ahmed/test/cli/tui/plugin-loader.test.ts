...
import type { TuiAttentionSoundPack } from "@ia-bot-ahmed/plugin/tui"
...
const { allThemes, addTheme } = await import("@ia-bot-ahmed/tui/context/theme")
...
`,
  ...
  process.env.IA_BOT_AHMED_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
  ...
  try {
    ...
    cwd.mockRestore()
    wait.mockRestore()
    delete process.env.IA_BOT_AHMED_PLUGIN_META_FILE
  }
})
...
test("does not bootstrap server plugins while initializing tui plugins", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "ia-bot-ahmed.json"), JSON.stringify({ plugin: [pathToFileURL(plugin).href] }))
      return { marker }
    },
  })
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.IA_BOT_AHMED_PLUGIN_META_FILE!,
    )
