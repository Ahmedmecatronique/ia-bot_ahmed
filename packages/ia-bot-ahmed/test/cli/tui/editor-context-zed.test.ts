...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@ia-bot-ahmed/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.IA_BOT_AHMED_ZED_DB
  process.env.IA_BOT_AHMED_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.IA_BOT_AHMED_ZED_DB
    else process.env.IA_BOT_AHMED_ZED_DB = previous
    home.mockRestore()
  }
})
