...
import { EffectFlock } from "@ia-bot-ahmed/core/util/effect-flock"
...
import { ConfigPluginV1 } from "@ia-bot-ahmed/core/v1/config/plugin"
...
const managedConfigDir = process.env.IA_BOT_AHMED_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.IA_BOT_AHMED_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.IA_BOT_AHMED_CONSOLE_TOKEN
  else process.env.IA_BOT_AHMED_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "ia-bot-ahmed.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "ia-bot-ahmed.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "ia-bot-ahmed.json") =>
...
it.effect("does not create global config when IA_BOT_AHMED_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    const custom = yield* tmpdirScoped()
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        "IA_BOT_AHMED_CONFIG_DIR",
        custom,
        Effect.gen(function* () {
          yield* Config.use.get().pipe(provideInstanceEffect(dir))

          expect(yield* FSUtil.use.existsSafe(path.join(dir, "ia-bot-ahmed.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
      ),
    )
  }),
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "ia-bot-ahmed.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "ia-bot-ahmed.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "ia-bot-ahmed.jsonc")
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(test.directory, "ia-bot-ahmed.json"))
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["ia-bot-ahmed"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "ia-bot-ahmed.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads agents from .ia-bot-ahmed/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
...
it.instance("loads commands from .ia-bot-ahmed/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
...
it.instance("loads commands from .ia-bot-ahmed/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
...
it.effect("does not try to install dependencies in read-only IA_BOT_AHMED_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("IA_BOT_AHMED_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable IA_BOT_AHMED_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "IA_BOT_AHMED_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
// Note: preload.ts sets IA_BOT_AHMED_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "ia-bot-ahmed.jsonc")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.ia-bot-ahmed/plugin/demo.ts", "file:///project/.ia-bot-ahmed/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.ia-bot-ahmed/plugin/demo.ts"])
  })
...
describe("IA_BOT_AHMED_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .ia-bot-ahmed/ directories when flag is set", () =>
    withProcessEnv(
      "IA_BOT_AHMED_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { IA_BOT_AHMED_CONFIG_DIR: undefined, IA_BOT_AHMED_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "IA_BOT_AHMED_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { IA_BOT_AHMED_DISABLE_PROJECT_CONFIG: "true", IA_BOT_AHMED_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed IA_BOT_AHMED_PERMISSION JSON used to crash
...
describe("IA_BOT_AHMED_PERMISSION env var", () => {
  it.instance("does not crash when IA_BOT_AHMED_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "IA_BOT_AHMED_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
describe("IA_BOT_AHMED_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in IA_BOT_AHMED_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
  ...
  it.instance("substitutes {file:} tokens in IA_BOT_AHMED_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
