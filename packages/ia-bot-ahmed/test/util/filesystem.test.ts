...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ia-bot-ahmed.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ia-bot-ahmed.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ia-bot-ahmed.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ia-bot-ahmed.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ia-bot-ahmed.json", "ia-bot-ahmed.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "ia-bot-ahmed.json"),
        path.join(project, "ia-bot-ahmed.jsonc"),
      ])
    })
  })
