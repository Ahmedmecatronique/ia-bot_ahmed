...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.IA_BOT_AHMED_TEST ?? "")'), {
      env: {
        IA_BOT_AHMED_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "IA_BOT_AHMED_TEST_SHELL"], {
      shell: true,
      env: {
        IA_BOT_AHMED_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("IA_BOT_AHMED_TEST_SHELL=ok")
  })
