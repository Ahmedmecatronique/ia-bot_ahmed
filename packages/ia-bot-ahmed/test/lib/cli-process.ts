// Subprocess test harness for the IaBotAhmed CLI. Spawns the real binary against
...
//
// Configuration flows through IaBotAhmed's built-in test affordances:
//   - IA_BOT_AHMED_CONFIG_CONTENT      : provider config inline, no files to find
//   - IA_BOT_AHMED_TEST_HOME           : pins os.homedir() â†’ tmpdir
//   - IA_BOT_AHMED_DISABLE_PROJECT_CONFIG : skip walking up for IaBotAhmed.json
//   - IA_BOT_AHMED_PURE                : skip external plugin discovery + install
//   - IA_BOT_AHMED_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `ia-bot-ahmed.run` is fully wired. The shape supports adding more
// builders (`ia-bot-ahmed.serve(opts)`, `ia-bot-ahmed.acp(opts)`, `ia-bot-ahmed.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape â€” see the TODO at the bottom of IaBotAhmedCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@ia-bot-ahmed/core/fs-util"
import { AppProcess } from "@ia-bot-ahmed/core/process"
...
const IaBotAhmedRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(IaBotAhmedRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    IA_BOT_AHMED_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    IA_BOT_AHMED_CONFIG_CONTENT: configJson,
    IA_BOT_AHMED_DISABLE_PROJECT_CONFIG: "1",
    IA_BOT_AHMED_PURE: "1",
    IA_BOT_AHMED_DISABLE_AUTOUPDATE: "1",
    IA_BOT_AHMED_DISABLE_AUTOCOMPACT: "1",
    IA_BOT_AHMED_DISABLE_MODELS_FETCH: "1",
    IA_BOT_AHMED_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `ia-bot-ahmed run`. New flags should
...
// `ia-bot-ahmed serve` is a long-lived process â€” it never exits on its own.
...
// `ia-bot-ahmed acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type IaBotAhmedCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `ia-bot-ahmed serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `ia-bot-ahmed acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly IaBotAhmed: IaBotAhmedCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    // cleanup â€” replaces the old mkdir + addFinalizer pair.
    const home = yield* fs.makeTempDirectoryScoped({ prefix: "oc-cli-" })

    const configJson = JSON.stringify(testProviderConfig(llm.url))
    const env = isolatedEnv(home, configJson)

    const spawn = Effect.fn("ia-bot-ahmed.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("ia-bot-ahmed.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "ia-bot-ahmed server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `ia-bot-ahmed serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("ia-bot-ahmed.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const IaBotAhmed: IaBotAhmedCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, IaBotAhmed })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `ia-bot-ahmed.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "ia-bot-ahmed") {
...
// every call site. Use this for any test that needs the IaBotAhmed CLI fixture.
//
...
// resources (e.g. `ia-bot-ahmed.serve`) without an extra `Effect.scoped` wrapper â€”
