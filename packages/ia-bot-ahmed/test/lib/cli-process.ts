// Subprocess test harness for the ia-bot-ahmed CLI. Spawns the real binary against
...
//
// Configuration flows through ia-bot-ahmed's built-in test affordances:
//   - IA_BOT_AHMED_CONFIG_CONTENT      : provider config inline, no files to find
//   - IA_BOT_AHMED_TEST_HOME           : pins os.homedir() → tmpdir
//   - IA_BOT_AHMED_DISABLE_PROJECT_CONFIG : skip walking up for ia-bot-ahmed.json
...
// different return shape — see the TODO at the bottom of ia-bot-ahmedCli.
...
const ia-bot-ahmedRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(ia-bot-ahmedRoot, "src/index.ts")
...
// `ia-bot-ahmed serve` is a long-lived process — it never exits on its own.
...
export type CliFixture = {
  ...
  readonly ia-bot-ahmed: ia-bot-ahmedCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const ia-bot-ahmed: ia-bot-ahmedCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, ia-bot-ahmed })
...
// every call site. Use this for any test that needs the ia-bot-ahmed CLI fixture.
//
...
// resources (e.g. `ia-bot-ahmed.serve`) without an extra `Effect.scoped` wrapper —
