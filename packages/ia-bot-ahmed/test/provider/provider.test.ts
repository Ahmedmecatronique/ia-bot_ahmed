...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  const item = providers[ProviderV2.ID.make("ia-bot-ahmed")]
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("cloudflare-ai-gateway")].options.metadata).toEqual({
      invoked_by: "test",
      project: "ia-bot-ahmed",
    })
  }),
  {
    config: {
      provider: { "cloudflare-ai-gateway": { options: { metadata: { invoked_by: "test", project: "ia-bot-ahmed" } } } },
    },
  },
)
...
it.effect("ia-bot-ahmed loader keeps paid models when config apiKey is present", () =>
  Effect.gen(function* () {
...
it.effect("ia-bot-ahmed loader keeps paid models when auth exists", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped()
