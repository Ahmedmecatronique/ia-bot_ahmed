...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.IA_BOT_AHMED_WEBSEARCH_PROVIDER

    try {
      process.env.IA_BOT_AHMED_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.IA_BOT_AHMED_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.IA_BOT_AHMED_WEBSEARCH_PROVIDER
      else process.env.IA_BOT_AHMED_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for ia-bot-ahmed or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.ia-bot-ahmed, { exa: false, parallel: false })).toBe(true)
