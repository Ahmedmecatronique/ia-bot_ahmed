...
import { LocationServiceMap } from "@ia-bot-ahmed/core/location-layer"
...
it.instance("plan agent denies edits except .ia-bot-ahmed/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".ia-bot-ahmed/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
`,
      ...
      const home = process.env.IA_BOT_AHMED_TEST_HOME
      process.env.IA_BOT_AHMED_TEST_HOME = test.directory
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          process.env.IA_BOT_AHMED_TEST_HOME = home
        }),
      )
