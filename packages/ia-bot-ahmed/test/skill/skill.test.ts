...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.IA_BOT_AHMED_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.IA_BOT_AHMED_TEST_HOME = prev
      }),
  )

describe("skill", () => {
  it.live("discovers skills from .ia-bot-ahmed/skill/ directory", () =>
...
`,
  ...
  it.live("discovers multiple skills from .ia-bot-ahmed/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => s.location !== "<built-in>")
          expect(list.map((s) => s.name)).toEqual(["ia-bot-ahmed-skill"])
        }),
      { git: true },
    ),
  )
