...
import { AccountV2 } from "@ia-bot-ahmed/core/account"
import { AccountTable } from "@ia-bot-ahmed/core/account/sql"
...
function withCreatedWorktree(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      expect(info).toMatchObject({ name, branch: "ia-bot-ahmed/api-test" })
      ...
      Effect.gen(function* () {
        ...
        const [consoleState, consoleOrgs, toolList, toolIDs, worktrees, resources] = yield* Effect.all(
          [
            ...
            request(`${ExperimentalPaths.tool}?provider=ia-bot-ahmed&model=gpt-5`, directory),
