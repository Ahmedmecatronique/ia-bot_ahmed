...
import { WorkspaceTable } from "@ia-bot-ahmed/core/control-plane/workspace.sql"
...
const originalEnv = {
  IA_BOT_AHMED_AUTH_CONTENT: process.env.IA_BOT_AHMED_AUTH_CONTENT,
  IA_BOT_AHMED_EXPERIMENTAL_WORKSPACES: process.env.IA_BOT_AHMED_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.IA_BOT_AHMED_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@ia-bot-ahmed.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.IA_BOT_AHMED_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=ia-bot-ahmed-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.IA_BOT_AHMED_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.IA_BOT_AHMED_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.IA_BOT_AHMED_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=ia-bot-ahmed-test")
