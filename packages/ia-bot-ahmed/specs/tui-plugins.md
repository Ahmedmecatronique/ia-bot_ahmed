...
- Author package entrypoint is `@ia-bot-ahmed/plugin/tui`.
...
```json
{
  "$schema": "https://ia-bot-ahmed.app/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/ia-bot-ahmed-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
    ...
    "sound": true,
    "volume": 0.4,
    "sound_pack": "ia-bot-ahmed.default",
    "sounds": {
...
- Import types from `@ia-bot-ahmed/plugin/tui`.
- `@ia-bot-ahmed/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@ia-bot-ahmed/plugin/tui"
...
- The value is a semver range checked against the running ia-bot-ahmed version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.ia-bot-ahmed` is absent, no check is performed (backward compatible).
...
- `ia-bot-ahmed plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `ia-bot-ahmed plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.ia-bot-ahmed` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.ia-bot-ahmed`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.ia-bot-ahmed`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.ia-bot-ahmed` dir or `IA_BOT_AHMED_CONFIG_DIR`, ia-bot-ahmed installs `@ia-bot-ahmed/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@ia-bot-ahmed/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus ia-bot-ahmed's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
ia-bot-ahmed registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, ia-bot-ahmed pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@ia-bot-ahmed/plugin/tui`.
...
- `message` is required; `title` defaults to `"ia-bot-ahmed"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `ia-bot-ahmed.default`.
...
- Local plugins persist installed themes under the local `.ia-bot-ahmed/themes` area near the plugin config source.
...
- `--pure` / `IA_BOT_AHMED_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.ia-bot-ahmed/plugins/tui-smoke.tsx`
- Local vim plugin: `.ia-bot-ahmed/plugins/tui-vim.tsx`
- Local smoke config: `.ia-bot-ahmed/tui.json`
- Local smoke theme: `.ia-bot-ahmed/plugins/smoke-theme.json`
