...
Trim `packages/ia-bot-ahmed/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/ia-bot-ahmed/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ia-bot-ahmed`.
- `[x]` `bun typecheck` from `packages/ia-bot-ahmed`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/ia-bot-ahmed/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ia-bot-ahmed`.
- `[x]` `bun dev generate > /tmp/ia-bot-ahmed-openapi.json` from `packages/ia-bot-ahmed`.
...
- Edit `packages/ia-bot-ahmed/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/ia-bot-ahmed-openapi.json` from `packages/ia-bot-ahmed`.
...
- Check source schemas in files like `packages/ia-bot-ahmed/src/session/schema.ts`, `packages/ia-bot-ahmed/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/ia-bot-ahmed/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/ia-bot-ahmed/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
