...
Delete `packages/ia-bot-ahmed/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ia-bot-ahmed/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ia-bot-ahmed/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ia-bot-ahmed`
- TUI plugin loader tests in `packages/ia-bot-ahmed` if runtime plugin API wiring changed
