...
Practical reference for a future `packages/server` split after the ia-bot-ahmed
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/ia-bot-ahmed`.
...
Until enough shared service code lives outside `packages/ia-bot-ahmed`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ia-bot-ahmed`

It should not import `packages/ia-bot-ahmed` services while `packages/ia-bot-ahmed`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on ia-bot-ahmed-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ia-bot-ahmed` implementation details.
