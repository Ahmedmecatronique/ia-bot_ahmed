...
Practical reference for the current tool-migration state in `packages/ia-bot-ahmed`.
...
Tool tests should use the existing Effect helpers in `packages/ia-bot-ahmed/test/lib/effect.ts`:
