...
import type { ProviderV2 } from "@ia-bot-ahmed/core/provider"
import { isContextOverflow } from "@ia-bot-ahmed/llm"
...
function message(providerID: ProviderV2.ID, e: APICallError) {
  return iife(() => {
    ...
    if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
      if (e.statusCode === 401) {
        return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `ia-bot-ahmed auth login <your provider URL>` to re-authenticate."
      }
      if (e.statusCode === 403) {
