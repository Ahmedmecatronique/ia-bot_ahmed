This is a plugin to simulate a remote environment locally. Add this to `.ia-bot-ahmed/ia-bot-ahmed.jsonc`:

```json
  "plugin": ["../packages/ia-bot-ahmed/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate ia-bot-ahmed server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/ia-bot-ahmed/script/run-workspace-server
```

With the plugin install, you can now run ia-bot-ahmed and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
