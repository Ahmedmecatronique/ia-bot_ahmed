...
import { InstallationLocal, InstallationVersion } from "@ia-bot-ahmed/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@ia-bot-ahmed/core/v1/config/console-state"
...
import { RemoteAuthError } from "@ia-bot-ahmed/core/v1/config/error"
import { ConfigPermissionV1 } from "@ia-bot-ahmed/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@ia-bot-ahmed/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["ia-bot-ahmed.jsonc", "ia-bot-ahmed.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.IA_BOT_AHMED_CONFIG && !Flag.IA_BOT_AHMED_CONFIG_DIR && !Flag.IA_BOT_AHMED_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ia-bot-ahmed.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ia-bot-ahmed.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "IA_BOT_AHMED_CONFIG_CONTENT") return "local"
        ...
        if (Flag.IA_BOT_AHMED_CONFIG) {
          yield* merge(Flag.IA_BOT_AHMED_CONFIG, yield* loadFile(Flag.IA_BOT_AHMED_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.IA_BOT_AHMED_CONFIG })
        }

        if (!Flag.IA_BOT_AHMED_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("ia-bot-ahmed", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.IA_BOT_AHMED_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from IA_BOT_AHMED_CONFIG_DIR", { path: Flag.IA_BOT_AHMED_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.ia-bot-ahmed/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.IA_BOT_AHMED_CONFIG_CONTENT) {
          const source = "IA_BOT_AHMED_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.IA_BOT_AHMED_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from IA_BOT_AHMED_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["IA_BOT_AHMED_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("IA_BOT_AHMED_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.IA_BOT_AHMED_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.IA_BOT_AHMED_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("IA_BOT_AHMED_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.IA_BOT_AHMED_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.IA_BOT_AHMED_DISABLE_PRUNE) {
