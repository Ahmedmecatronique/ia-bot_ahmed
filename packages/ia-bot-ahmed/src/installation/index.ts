...
import { makeRuntime } from "@ia-bot-ahmed/core/effect/runtime"
import semver from "semver"
import { InstallationChannel, InstallationVersion } from "@ia-bot-ahmed/core/installation/version"
import { NpmConfig } from "@ia-bot-ahmed/core/npm-config"
...
export function userAgent(client = "cli") {
  return `ia-bot-ahmed/${InstallationChannel}/${InstallationVersion}/${client}`
}
...
export class Service extends Context.Service<Service, Interface>()("@ia-bot-ahmed/Installation") {}
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          { name: "brew", command: () => text(["brew", "list", "--formula", "ia-bot-ahmed"]) },
          { name: "scoop", command: () => text(["scoop", "list", "ia-bot-ahmed"]) },
          { name: "choco", command: () => text(["choco", "list", "--limit-output", "ia-bot-ahmed"]) },
        ]
        ...
        for (const check of checks) {
          ...
          const installedName =
            check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "ia-bot-ahmed" : "ia-bot-ahmed-ai"
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        const response = yield* httpOk.execute(
          HttpClientRequest.get("https://api.github.com/repos/anomalyco/ia-bot-ahmed/releases/latest").pipe(
            HttpClientRequest.acceptJson,
          ),
        )
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          case "curl":
            ...
            break
          case "npm":
            upgradeResult = yield* run(["npm", "install", "-g", `ia-bot-ahmed-ai@${target}`])
            break
          case "pnpm":
            upgradeResult = yield* run(["pnpm", "install", "-g", `ia-bot-ahmed-ai@${target}`])
            break
          case "bun":
            upgradeResult = yield* run(["bun", "install", "-g", `ia-bot-ahmed-ai@${target}`])
            break
          case "brew": {
            const formula = yield* getBrewFormula()
          ...
          case "choco":
            upgradeResult = yield* run(["choco", "upgrade", "ia-bot-ahmed", `--version=${target}`, "-y"])
            break
          case "scoop":
            upgradeResult = yield* run(["scoop", "install", `ia-bot-ahmed@${target}`])
            break
          default:
