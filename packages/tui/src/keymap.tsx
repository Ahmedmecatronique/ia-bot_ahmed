...
export const IA_BOT_AHMED_BASE_MODE = "base"
...
export function createIaBotAhmedModeStack(keymap: OpenTuiKeymap) {
  ...
  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(IA_BOT_AHMED_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(IA_BOT_AHMED_MODE_KEY, stack.at(-1)?.mode ?? IA_BOT_AHMED_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? IA_BOT_AHMED_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(IA_BOT_AHMED_MODE_KEY, undefined)
