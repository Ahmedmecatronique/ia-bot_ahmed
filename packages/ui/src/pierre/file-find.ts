...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("ia-bot-ahmed-find")
  api.delete("ia-bot-ahmed-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("ia-bot-ahmed-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("ia-bot-ahmed-find", new Highlight(...rest))
    return true
  }
