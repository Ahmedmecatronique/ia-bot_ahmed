...
Set `IA_BOT_AHMED_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
IA_BOT_AHMED_PERFORMANCE_TRACE_DIR=/tmp/ia-bot-ahmed-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `IA_BOT_AHMED_PERFORMANCE_SELECTOR_TRACE=1`.
