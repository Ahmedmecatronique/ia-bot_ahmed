import type {
  ...
  Session,
  Todo,
} from "@ia-bot-ahmed/sdk/v2/client"
...
import { NormalizedProviderListResponse } from "@ia-bot-ahmed/ui/context"
    ...
    queryFn: () =>
      retry(() =>
        sdk.project.list().then((x) => {
          return (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("ia-bot-ahmed-test"))
            .slice()
