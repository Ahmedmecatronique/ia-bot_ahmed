...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("ia-bot-ahmed://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("ia-bot-ahmed://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("ia-bot-ahmed://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("ia-bot-ahmed://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("ia-bot-ahmed://open-project")).toBeUndefined()
    expect(parseDeepLink("ia-bot-ahmed://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "ia-bot-ahmed://open-project?directory=/a",
      "ia-bot-ahmed://other?directory=/b",
      "ia-bot-ahmed://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("ia-bot-ahmed://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("ia-bot-ahmed://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("ia-bot-ahmed://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("ia-bot-ahmed://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "ia-bot-ahmed://new-session?directory=/a",
      "ia-bot-ahmed://open-project?directory=/b",
      "ia-bot-ahmed://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __IA_BOT_AHMED__: {
        deepLinks: ["ia-bot-ahmed://open-project?directory=/a"],
      },
    } as unknown as Window & { __IA_BOT_AHMED__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["ia-bot-ahmed://open-project?directory=/a"])
