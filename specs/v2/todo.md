...
The ia-bot-ahmed server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@ia-bot-ahmed/llm` tool loop after replacing its
...
- plugins get global "ia-bot-ahmed" instance like in that post i showed
- ia-bot-ahmed instance has stuff like `ia-bot-ahmed.session.prompt()` or
  `ia-bot-ahmed.tool.register({...})`
...
- decide whether to preserve deprecated `@ia-bot-ahmed/llm` orchestration exports
