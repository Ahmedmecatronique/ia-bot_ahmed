...
Both `action` and `resource` use ia-bot-ahmed's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.ia-bot-ahmed` files is intentionally deferred until `.ia-bot-ahmed` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether ia-bot-ahmed permits an operation through its managed execution paths.
