...
Move the canonical ia-bot-ahmed terminal application from
`packages/ia-bot-ahmed/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/tui
name: @ia-bot-ahmed/tui
```

Target dependency graph:

```text
packages/ia-bot-ahmed ---\
                      > @ia-bot-ahmed/tui -> @ia-bot-ahmed/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/ia-bot-ahmed`,
`packages/cli`, or `@ia-bot-ahmed/core`.

The SDK is the TUI's ia-bot-ahmed boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/ia-bot-ahmed`.
- Do not replace private `packages/ia-bot-ahmed` imports with `@ia-bot-ahmed/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@ia-bot-ahmed/tui`.

## Ownership Boundary

### `@ia-bot-ahmed/tui` Owns
...
- ia-bot-ahmed domain data displayed by the TUI
...
```text
packages/ia-bot-ahmed/src/cli/cmd/tui
```

Its private dependency on `packages/ia-bot-ahmed` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/ia-bot-ahmed/src/*`.
...
Status: Completed. The private `@ia-bot-ahmed/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@ia-bot-ahmed/tui`.
...
- The package has no dependency on `ia-bot-ahmed`, `@ia-bot-ahmed/cli`, or
  `@ia-bot-ahmed/core`.
...
primitives now live in `@ia-bot-ahmed/tui`. The legacy host consumes explicit
...
and build inputs through `@ia-bot-ahmed/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads ia-bot-ahmed globals or process state;
...
Replace process-global ia-bot-ahmed state with resolved TUI inputs.
...
- Keep environment reads needed by legacy command or worker startup in
  `packages/ia-bot-ahmed` adapters.
...
`@ia-bot-ahmed/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/ia-bot-ahmed` initially.
...
canonical in `@ia-bot-ahmed/tui`. Configured references resolve through the new
...
adapters rather than ia-bot-ahmed domain implementations.

Make the SDK the only ia-bot-ahmed domain boundary used by the TUI.
...
- Domain-facing TUI code imports ia-bot-ahmed data and operations only from
  `@ia-bot-ahmed/sdk`.
...
`@ia-bot-ahmed/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/ia-bot-ahmed`.
...
- The package has no imports from `packages/ia-bot-ahmed`, `packages/cli`, or
  `@ia-bot-ahmed/core`.
...
public `@ia-bot-ahmed/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/ia-bot-ahmed`.
- Keep the legacy embedded worker and server startup in `packages/ia-bot-ahmed`.
- Change those adapters to load config, create transport inputs, and call the
  public `@ia-bot-ahmed/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/ia-bot-ahmed`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@ia-bot-ahmed/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@ia-bot-ahmed/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for ia-bot-ahmed domain state.
...
cd packages/ia-bot-ahmed && bun typecheck
...
rg '@ia-bot-ahmed/core|packages/ia-bot-ahmed|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/ia-bot-ahmed/src packages/ia-bot-ahmed/test
```
...
- Build the current-platform `packages/ia-bot-ahmed` binary.
