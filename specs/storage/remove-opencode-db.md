# Remove `packages/ia-bot-ahmed/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/ia-bot-ahmed/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the ia-bot-ahmed legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/ia-bot-ahmed/src/storage/db.ts` are concentrated in 22 source files:

- `packages/ia-bot-ahmed/src/account/repo.ts`
- `packages/ia-bot-ahmed/src/cli/cmd/db.ts`
- `packages/ia-bot-ahmed/src/cli/cmd/import.ts`
- `packages/ia-bot-ahmed/src/cli/cmd/stats.ts`
- `packages/ia-bot-ahmed/src/control-plane/workspace.ts`
- `packages/ia-bot-ahmed/src/index.ts`
- `packages/ia-bot-ahmed/src/node.ts`
- `packages/ia-bot-ahmed/src/permission/index.ts`
- `packages/ia-bot-ahmed/src/project/project.ts`
- `packages/ia-bot-ahmed/src/server/projectors.ts`
- `packages/ia-bot-ahmed/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/ia-bot-ahmed/src/server/shared/fence.ts`
- `packages/ia-bot-ahmed/src/session/message-v2.ts`
- `packages/ia-bot-ahmed/src/session/projectors.ts`
- `packages/ia-bot-ahmed/src/session/prompt.ts`
- `packages/ia-bot-ahmed/src/session/session.ts`
- `packages/ia-bot-ahmed/src/session/todo.ts`
- `packages/ia-bot-ahmed/src/share/share-next.ts`
- `packages/ia-bot-ahmed/src/storage/db.ts`
- `packages/ia-bot-ahmed/src/sync/index.ts`
- `packages/ia-bot-ahmed/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy ia-bot-ahmed database wrapper; `packages/ia-bot-ahmed/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the ia-bot-ahmed wrapper.
...
Status: Completed. `SyncEvent` and the ia-bot-ahmed projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy ia-bot-ahmed database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy ia-bot-ahmed database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy ia-bot-ahmed database wrapper.
...
All migration groups are complete or superseded. `packages/ia-bot-ahmed/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No ia-bot-ahmed data-migration group remains.

The previous ia-bot-ahmed `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate ia-bot-ahmed data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/ia-bot-ahmed`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/ia-bot-ahmed/src`
- `bun typecheck` from `packages/ia-bot-ahmed`
- Relevant package tests from `packages/ia-bot-ahmed`, not the repo root
