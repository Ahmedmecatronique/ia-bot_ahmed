...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an ia-bot-ahmed storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/ia-bot-ahmed` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No ia-bot-ahmed paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@ia-bot-ahmed/effect-drizzle-sqlite`
...
- ia-bot-ahmed-specific path/channel selection stays in `packages/ia-bot-ahmed`.

## ia-bot-ahmed Adoption Notes

These are not package requirements, but they matter for the later ia-bot-ahmed adoption PR.

The current `packages/ia-bot-ahmed/src/storage/db.ts` has two non-obvious semantics that the ia-bot-ahmed wrapper must preserve when it consumes this adapter:
...
The ia-bot-ahmed wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving ia-bot-ahmed to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@ia-bot-ahmed/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@ia-bot-ahmed/effect-drizzle-sqlite` as a dependency of `packages/ia-bot-ahmed`.
5. Port `packages/ia-bot-ahmed/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus ia-bot-ahmed-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of ia-bot-ahmed's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then ia-bot-ahmed can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay ia-bot-ahmed-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/ia-bot-ahmed`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not ia-bot-ahmed domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/ia-bot-ahmed` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing ia-bot-ahmed's current database runtime.
